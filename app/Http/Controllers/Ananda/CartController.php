...
class CartController extends Controller {
	...
	public function __construct() {
		$this->middleware('auth', ['except' => ['order_receipt', 'get_print_receipt', 'rt_log']]);
	}
	...
	public function add_to_cart(Request $request) {
		...
		$tax_details = $this->taxCal($barcode, $qty, $s_price, $product_response->tax_code);

		// dd($tax_details);

		// dd($product_response);
		// $getvrid = DB::table('ananda.invitem')->select('ICODE')->where('BARCODE', $barcode)->first();
		// if (empty($getvrid)) {
		...
		// $item = DB::table('ananda.invitem')->select('GRPCODE', 'INVARTICLE_CODE', 'CCODE1', 'CCODE2', 'CCODE3', 'CCODE4', 'CCODE5', 'CCODE6', 'ICODE', 'GENERATED', 'MRP', 'CNAME1', 'CNAME2', 'INVHSNSACMAIN_CODE')->Where('ICODE', $barcode)->first();
		...
		$cart_id = DB::table('cart')->insertGetId([
			...
			'subclass_id' => $product_response->section_code,
		...
		return response()->json(['status' => 'add_to_cart', 'message' => 'Product added successfully.', 'total_qty' => $carts->sum('qty'), 'total_amount' => $carts->sum('total'),
		], 200);
	}
	...
	public function cart_details(Request $request) {
		...
		$cart = DB::table('cart')->where('user_id', $c_id)->where('v_id', $v_id)->where('store_id', $store_id)->where('status', 'process')->orderBy('updated_at','DESC')->get();
		...
		foreach ($cart as $key => $value) {
			...
			// dd($response);
			$salesman_name ='';
			$salesmans = Vendor::where('vu_id', $value->salesman_id)->first();
			if($salesmans){
			...
			$data[] = array(
				...
				'product_data' => [
					...
					'image_list' => [ $product_details->images],
	...
	public function payment_details(Request $request) {
		...
		if ($status == 'success') {
			...
			return response()->json(['status' => 'payment_save', 'redirect_to_qr' => true, 'message' => 'Save Payment', 'data' => $payment, 'order_summary' => $order_arr], 200);
	...
	public function order_details(Request $request) {
		...
		$o_id = Invoice::where('invoice_id', $order_id)->where('v_id', $v_id)->where('store_id', $store_id);
		...
		foreach ($carts as $key => $value) {
			$applied_offer = [];
			$available_offer = [];
			$item_details = anandaCategory($value->barcode);
			if ($value->pdata == '""') {
				$offer = 'No';
				$offer_data = (object) [];
			} else {
				...
				$promo_buy = DB::table('ananda.promo_buy')->select('ASSORTMENT_NAME')->where('PROMO_CODE', $offerdata['promo_code'])->first();
				if ($promo_buy) {
	...
	public function get_carry_bags(Request $request) {
		...
		$carry_bags = DB::table('ananda.invitem')->select('ICODE as BAG_ID', 'CNAME1 as Name', 'MRP as Price')->whereIn('ICODE', $carr_bag_arr)->get();
	...
	public function save_carry_bags(Request $request) {
		...
		foreach ($bags as $key => $value) {
			...
			$price_master = DB::table('ananda.invitem')->select('ICODE as BAG_ID', 'CNAME1 as Name', 'MRP as Price')->where('ICODE', $value[0])->first();
			...
			if ($exists) {
				...
				if ($value[1] < 1) {
					...
					$request->request->add(['barcode' => $value[0], 'qty' => $value[1], 'unit_mrp' => $price_master->Price, 'r_price' => $price_master->Price * $value[1], 's_price' => $price_master->Price * $value[1], 'discount' => 0, 'pdata' => '', 'data' => $data, 'ogbarcode' => $value[0]]);
...
//         ini_set("display_errors","On");
       ...
       // dd($store_id);
		$site_details = DB::table('ananda.admsite')->where('CODE', $store->mapping_store_id)->first();
		//dd($site_details);
...
//			}
        ...
        $printInvioce = new PrintInvoice('SUNMI');
        $printInvioce->addLineCenter($site_details->NAME, 27, true);
        $printInvioce->addLine($site_details->ADDRESS, 24);
        $printInvioce->addLine('Helpline No.:'.$store->helpline, 24);
        $printInvioce->addLine('E-mail: '.$store->email, 24);
        $printInvioce->addLine('GSTIN: '.$store->gst, 24);
        $printInvioce->addDivider('-', 20);
        $printInvioce->addLine('TAX INVOICE', 24, true);
        $printInvioce->addDivider('-', 20);

        $printInvioce->addLineLeft('Invoice No.:'.$order_details->invoice_id, 24, true);
        $printInvioce->addLine('Cashier:'.$order_details->vuser->first_name.' '.$order_details->vuser->last_name, 24);
        $printInvioce->addLine('Date:'.date('d-M-Y', strtotime($order_details->created_at)), 24);
        $printInvioce->addLine('Time:'.date('h:i A', strtotime($order_details->created_at)), 24);
        $printInvioce->addLine('GST Doc No.:'.$order_details->custom_order_id, 24);

        $printInvioce->addDivider('-', 20);
        $printInvioce->tableStructure(['Sr.No', 'QTY', 'Rate', 'Disc', 'Amount'], [1, 1, 1, 1, 1], 20);
        $printInvioce->leftRightStructure('Product', 'HSN Code', 20);
        $printInvioce->addDivider('-', 20);
        for($i = 0; $i < count($product_data); $i++) {
            if($i % 2 == 0) {
                $printInvioce->tableStructure([$product_data[$i]['sr_no'], $product_data[$i]['qty'], $product_data[$i]['rate'], $product_data[$i]['discount'], $product_data[$i]['total']],
                    [1, 1, 1, 1, 1], 20);
            } else {
                $printInvioce->leftRightStructure($product_data[$i]['name'], $product_data[$i]['hsn'], 20);
            }
        }
        $printInvioce->addDivider('-', 20);
        $printInvioce->tableStructure(['Total', $cart_qty, $total_amount], [2, 1, 5], 22);
        $printInvioce->addDivider('-', 20);

        $printInvioce->numToWords($order_details->total, 22);

        $printInvioce->addDivider('-', 20);
        $printInvioce->leftRightStructure('Code', $total_amount, 22);
        $printInvioce->addDivider('-', 20);

        $printInvioce->addLineLeft("Customer Paid: ".format_number($cash_collected), 22, true);
        $printInvioce->addLine("Balance Refund: ".format_number($cash_return), 22, true);

        $printInvioce->addDivider('-', 20);
        $printInvioce->leftRightStructure('Total Sale: ', $order_details->total, 22, true);
        $printInvioce->leftRightStructure('Net Payable: ', $order_details->total, 22, true);

        if(!empty($final_gst)) {

            $printInvioce->addLineLeft('\n' . "GST Summary", 22);

            $printInvioce->addDivider('-', 20);
            $printInvioce->tableStructure(['Descriptor', 'Taxable', 'CGST', 'SGST'], [2, 1, 1, 1], 20);

            foreach ($final_gst as $index => $gst) {
                $printInvioce->tableStructure([$gst->name, $gst->taxable, $gst->cgst, $gst->sgst],
                    [2, 3, 2, 2], 22);
            }


            $printInvioce->addDivider('-', 20);
            $printInvioce->tableStructure(['Total ', format_number($taxable_amount), format_number($total_csgt), format_number($total_sgst)],
                [2, 3, 2, 2], 22, true);
        }
        $printInvioce->addDivider('-', 20);
...
//        print($printInvioce->getFinalResult()); die;
	...
	public function taxCal($barcode, $qty, $mrp, $tax_code)
	{
		$data = array();
		$cgst_amount = 0;
		$sgst_amount = 0;
		...
		$to_amount = 0;
		$tax_name = '';
		$main = DB::table('ananda.invhsnsacmain')->select('HSN_SAC_CODE')->where('CODE', $tax_code)->first();
		$det = DB::table('ananda.invhsnsacdet')->select('CODE','INVGSTRATE_CODE','SLAB_APPL','SLAB_BASIS')->where('INVHSNSACMAIN_CODE', $tax_code)->orderBy('CODE', 'desc')->first();
		...
		if (!empty($det)) {
			...
			if ($det->SLAB_APPL == 'Y') {
	            ...
	            $slabs = DB::table('ananda.invhsnsacslab')->select('INVGSTRATE_CODE', 'AMOUNT_FROM')->where('INVHSNSACMAIN_CODE', $tax_code)->where('INVHSNSACDET_CODE', $det->CODE)->orderBy('AMOUNT_FROM','ASC')->get()->toArray();
	            ...
	            $gst = DB::table('ananda.invgstrate')->where('CODE', $lowest_invgst_rate_code)->first();
	            ...
	            foreach($range as $keyR => $value){
	                if($total_taxable_amount >= $value['from'] && $total_taxable_amount < $value['to']){
	                    ...
	                    $gst = DB::table('ananda.invgstrate')->where('CODE', $invGstRateCode)->first();
	                    //dd($gst);
	        	...
	        	$mrp = $mrp/$qty;
	            $gst = DB::table('vmart.invgstrate')->where('CODE', $det->INVGSTRATE_CODE)->first();
	            $slab_cgst_amount = $mrp / ( 100 + $gst->CGST_RATE + $gst->SGST_RATE + $gst->CESS_RATE ) * $gst->CGST_RATE;
	            $slab_sgst_amount = $mrp / ( 100 + $gst->CGST_RATE + $gst->SGST_RATE + $gst->CESS_RATE ) * $gst->SGST_RATE;
	            $slab_cess_amount = $mrp / ( 100 + $gst->CGST_RATE + $gst->SGST_RATE + $gst->CESS_RATE ) * $gst->CESS_RATE;
	            $cgst = $gst->CGST_RATE;
