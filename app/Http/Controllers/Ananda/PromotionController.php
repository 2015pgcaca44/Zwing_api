...
class PromotionController extends Controller {
	public function final_check_promo_sitewise($request, $cart = 0) {
		...
		$product_default_image = $vendorS->getProductDefaultImage(['v_id' => $v_id, 'trans_from' => $trans_from, 'store_id' => $store_id,  'user_id' => $user_id, 'role_id' => $role_id , 'udidtoken'=>$udidtoken]);
		$multiple_mrp_flag = false;
		$multipleMrp =[];
		if(env('APP_ENV') == 'development'){

			if($id == 'GAP1054' || $id == 'GAP1055' ){
				$multiple_mrp_flag = true;
				$multipleMrp = [ $this->format_and_string($item->MRP) ,  
					$this->format_and_string($item->MRP +1),
				];
			}
		}else{
			$multiple_mrp_flag = false;
		...
		if ($cart == 1) {
			$push_data = [
				...
				'multiple_price_flag' => $multiple_mrp_flag,
				'multiple_mrp' => $multipleMrp,
	...
	public function getAllBarcodeByAssort($array) {
		...
		if ($cloumn_name == 'ICODE') {
			...
			if ($cloumn_name != 'DIVISION_GRPCODE' && $cloumn_name != 'SECTION_GRPCODE' && $cloumn_name != 'DEPARTMENT_GRPCODE') {
				...
				$get_art_from_depart = DB::table('ananda.invarticle')->where('GRPCODE', end($values))->get()->pluck('CODE');
				$inbar = DB::table('ananda.invitem')->whereIn('INVARTICLE_CODE', $get_art_from_depart)->get(['ICODE'])->toArray();
				// dd($inbar);
				return $inbar;
			}
		}
