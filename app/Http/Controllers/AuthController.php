...
class AuthController extends Controller
{
    ...
    public function profile_update(Request $request)
    {
        ...
        return response()->json(['status' => 'profile_update', 'message' => 'Profile Updated Successfully', 'mobile' => $mobile, 'c_id' => $c_id, 'api_token' => User::find($c_id)->api_token], 200);
    }
    ...
    public function retryFailedJobs(Request $request)
    {
        ...
        $checkExits = FailedJob::find($request->id);

        if(!empty($checkExits)) {
            Artisan::call('queue:retry '.$request->id);
            return response()->json([ 'status' => 'success', 'message' => 'Job retry successfully.' ]);
        } else {
            return response()->json([ 'status' => 'fail', 'message' => 'Queue not found.' ]);
        }
    }

    public function retryInboundJobs(Request $request)
    {
        $this->validate($request, [
            'id'    => 'required',
            'type'  => 'required',
        ...
        $itemRequest = new \Illuminate\Http\Request();
        $itemRequest->merge(['v_id' => (int)$request->v_id, 'client_id' => $request->client_id, 'ack_id' => $request->ack_id ]);
        $itemC = new  \App\Http\Controllers\ItemController;
        // dd($itemRequest->all());
        $response = $itemC->processItemMasterCreationJob($itemRequest);
        // dd($response);
