...
//  Using Ginesys's Cart controller as parent controller
	...
	public function get_print_receipt(Request $request) 
	{
		...
		$gst   = '';
		$vendorDetail = VendorDetails::where('v_id',$store->v_id)->first();
		if($vendorDetail){
			$gst = $vendorDetail->gst_no;
		}
		...
		foreach ($cart_product as $key => $value) {
				...
				if ($order_details->transaction_type == 'sales') {
					array_push($product_data, [
						...
						'rsp' => round($value->unit_mrp),
						'tax_amt' => round($value->tax,2),
		...
		$terms_conditions =  array('1. No Exchange For Accessories & Under Garments.','2. No Exchange For Winter & Discounted Items.','3. Goods Can Be Exchange For The Same.','4. You Can Exchange Product Within 30 Days Only.','5. Return Amount Should be Redeem on Same Day.');
        ...
        // Start center
		$printInvioce->addLineCenter('BAAZAR KOLKATA', 24,true);
		$printInvioce->addLine('A Unit of BAAZAR KOLKATA RETAIL \nLIMITED', 22);
        ...
        $printInvioce->addLineLeft('GST Doc No.:'.$order_details->custom_order_id, 22);
 		$printInvioce->addLineLeft($order_details->invoice_id , 22,true);
 		$printInvioce->addLineLeft(date('h:i A', strtotime($order_details->created_at)).' '.date('d-M-Y', strtotime($order_details->created_at)) , 22);
        ...
        $printInvioce->addLineLeft('Customer Mobile: '.(string) $order_details->user->mobile, 22);
        // Closes Left & Start center
        $printInvioce->addDivider('-', 20);

        $printInvioce->tableStructure(['SL', 'Product Description', 'HSN'], [3, 24,6], 22);
          
        $printInvioce->tableStructure(['Qty','Rate','DISC','RSP','TAX', '& %','Amount'], [4,5,5,5,5,3,7], 22);
        $printInvioce->addDivider('-', 20);
		...
		$printInvioce->leftRightStructure('Round Off', format_number($roundoffamt), 22); //$due
		$printInvioce->leftRightStructure('Due', format_number('0.00'), 22);
        $printInvioce->addLineLeft('Rs: '.ucfirst(numberTowords(round($order_details->total))).' Only' , 22);
        $printInvioce->leftRightStructure('GST', format_number('0.00'), 22);
        $printInvioce->addDivider('-', 20);
		...
		$printInvioce->tableStructure(['Total Sale', $cart_qty, $total_amount], [12, 4, 18], 22);
