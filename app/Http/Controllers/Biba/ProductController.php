...
class ProductController extends Controller
{
    ...
    public function check_product_exist_in_cart(Request $request){
        ...
        $cart = TempCart::where('store_id', $store_id)->where('v_id', $v_id)->where('order_id', $order_id)->where('user_id', $c_id)->where('status', 'process')->get();

        $product_qty = TempCart::where('store_id', $store_id)->where('v_id', $v_id)->where('order_id', $order_id)->where('user_id', $c_id)->where('barcode', $barcode)->where('status', 'process')->sum('qty');
    ...
    public function product_details(Request $request)
    {
        ...
        $product_data['unit_csp'] = $offer_data['unit_csp'];
       ...
       $product_data['discount'] = format_number($offer_data['total_discount']);
        ...
        //This condition is added to directly adding and updating to cart 
        //This condition is added to directly adding and updating to cart 
        if($trans_from == 'ANDROID_VENDOR' || $trans_from == 'IOS_VENDOR' ){
                
            $request->request->add([ 'qty' => $product_data['qty'] , 
                    'unit_mrp' => $product_data['unit_mrp'],
                    'unit_csp' => $product_data['unit_csp'],
                    'r_price' => $product_data['r_price'],
                    's_price' => $product_data['s_price'],
                    'discount' => $product_data['discount'],
                    'pdata' => $product_data['pdata'],
                    'weight_flag' => $product_data['weight_flag']
