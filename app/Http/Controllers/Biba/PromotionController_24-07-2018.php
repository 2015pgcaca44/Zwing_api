           ...
           // $all_sources = array_unique($list_ru_prdv->pluck('ID_ITM')->all() );
            ...
            foreach ($final_product as $key => $value) {
                if ($value['is_promo'] == 1) {
                    ...
                    foreach($single_list_ru_prdv as $sources){
                        $cartItem = $carts->where('item_id', $sources->ID_ITM)->first();
                        if($cartItem){

                            $loopQty = $sources->QU_TH;
                            while($loopQty > 0){
                                $cartItems[] = ['item_id' => $cartItem->item_id ,'qty' => 1 , 'unit_mrp' => $cartItem->unit_mrp];
                                $loopQty--;
                            } 
                            ...
                            if(!$cart_item){
                                $cartItems[] = ['item_id' => $sources->ID_ITM ,'qty' => 1 , 'unit_mrp' => $mrp];
                            }
                        }   
                    }
                    $cartItems = collect($cartItems)->sortByDesc('unit_mrp');
