     ...
     public function dataFetchRequest()
    {
    	...
    	// Creating an array for request
		   ...
		   foreach ($data['ConcessionItems'] as $itemKeys => $itemValues) {
                ...
                $itemPrice = $itemValues['PriceInCents']/100;
			 $array_data = array(
			 	'name'=>$itemValues['Description'],
			 	'auto_sku' => true,
                'v_id' => $this->v_id,
                'department'=> '{"name":"cinepolis"}',
			 	'brand' => '{"name":"cinepolis"}',
                'category' => ['{"name":"Concessions","code":40}'],
			 	...
			 	'variant_products'=> ['{"vrname":"default","vrbarcode":"'.$itemValues['Id'].'","vrqty":0,"sku":"","hsn_code":null,"tax_group_id":null,"variant_prices":[{"id":null,"mrp":"'.$itemPrice.'","rsp":"'.$itemPrice.'","special_price":"'.$itemPrice.'"}]}']
			 );
...
//            dd($uom_conversion);
            ...
            $this->productconfig->vendorItemMapping($product->id, $request->v_id);
