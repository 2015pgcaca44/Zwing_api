...
{
	public function get_creditnote_recipt(Request $request){
        ...
        $cart_product  = Voucher::leftJoin('dep_rfd_trans','cr_dr_voucher.ref_id','dep_rfd_trans.trans_src_ref')->where('cr_dr_voucher.v_id',$order_details->v_id)->where('cr_dr_voucher.store_id',$order_details->store_id)->where('cr_dr_voucher.user_id', $order_details->user_id)->get();
        ...
        $printInvioce->tableStructure(['#', 'Document No','Credit Note No'], [3,20,16], 22);
        ...
        $printInvioce->addLineCenter('Remarks'.$order_details->remark, 22, true);
