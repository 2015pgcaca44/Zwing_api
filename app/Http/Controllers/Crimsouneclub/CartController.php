...
class CartController extends Controller
{
    ...
    public function add_to_cart(Request $request)
    {
        ...
        if (is_string($barcode)) {
          $getvrid = DB::table('crimsouneclub.invitem')->where('ICODE', $barcode)->first();
        } else {
          $getvrid = DB::table('crimsouneclub.invitem')->where('BARCODE', $barcode)->first();
        }
        ...
        $item = DB::table('crimsouneclub.invitem')->select('GRPCODE','INVARTICLE_CODE','CCODE1','CCODE2','CCODE3','CCODE4','CCODE5','CCODE6','ICODE','GENERATED','MRP','CNAME1','CNAME2','INVHSNSACMAIN_CODE')->Where('ICODE', $barcode)->first();
        ...
        $taxmain = DB::table('crimsouneclub.invhsnsacmain as main')
            ...
            if ($taxmain->SLAB_APPL == 'Y') {
                ...
                $onlytax = DB::table('crimsouneclub.invgstrate')->select('TAX_NAME','CGST_RATE','IGST_RATE','SGST_RATE','CESS_RATE')->where('CODE', $taxmain->INVGSTRATE_CODE)->first();
    ...
    public function add_to_cart_by_qty($request)
    {
        ...
        $pdata = json_decode($pdata);
        // dd($pdata);
        $getvrid = DB::table('crimsouneclub.invitem')->select('ICODE')->where('BARCODE', $barcode)->first();
        if (empty($getvrid)) {
            $barcode = $request->barcode;
        } else {
            $barcode = $getvrid->ICODE;
    ...
    public function order_details(Request $request)
    {
        ...
        foreach ($carts as $key => $value) {
            $applied_offer = [];
            $available_offer = [];
            $item_details = crimsouneclubCategory($value->barcode);
            if ($value->pdata == '""') {
                $offer = 'No';
                $offer_data = (object)[];
            } else {
                ...
                $promo_buy = DB::table('crimsouneclub.promo_buy')->select('ASSORTMENT_NAME')->where('PROMO_CODE', $offerdata['promo_code'])->first();
                if($promo_buy){
    ...
    public function get_carry_bags(Request $request)
    {
        ...
        $carr_bag_arr =  [ 'VR132797', 'VR132799' ,'VR132807'];
        $carry_bags = DB::table('crimsouneclub.invitem')->select('ICODE as BAG_ID','CNAME1 as Name', 'MRP as Price')->whereIn('ICODE', $carr_bag_arr)->get();
    ...
    public function save_carry_bags(Request $request)
    {
        ...
        foreach ($bags as $key => $value) {
            ...
            $price_master = DB::table('crimsouneclub.invitem')->select('ICODE as BAG_ID','CNAME1 as Name', 'MRP as Price')->where('ICODE', $value[0])->first();
            if($exists) {
    ...
    public function get_print_receipt(Request $request)
    {
        ...
        $site_details = DB::table('crimsouneclub.admsite')->where('CODE', $store->mapping_store_id)->first();
        ...
        $terms_conditions =  array('(1) Exchange Within 7 days only.','(2) Without Tag Exchange not Accepted','(3) No Cash Refund','(4) No Exchange or Refund in Sale Period','(5) Worn Goods will not be entertained','(6) MRP Are Inclusive of Applicable Tax');
        $data = [
            ...
            'date'                  => date('d-M-Y', strtotime($order_details->created_at))." at ".date('h:i:s A', strtotime($order_details->created_at)),
