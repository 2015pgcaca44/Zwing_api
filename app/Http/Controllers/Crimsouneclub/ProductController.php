...
class ProductController extends Controller
{
    ...
    public function product_details(Request $request)
    {
        ...
        if (is_string($barcode)) {
          $item = DB::table('crimsouneclub.invitem')->where('ICODE', $barcode)->first();
        } else {
          $item = DB::table('crimsouneclub.invitem')->where('BARCODE', $barcode)->first();
        }
        // dd($item);
        if(!$item) {
            $itema = DB::table('crimsouneclub.invitem')->where('ICODE', $barcode)->first();
            if (!$itema) {
