...
namespace App\Http\Controllers\Crimsouneclub;
...
class PromotionController extends Controller
{
    public function final_check_promo_sitewise($request, $cart = 0)
    {
        ...
        $count = 0;
        $item = DB::table('crimsouneclub.invitem')->select('GRPCODE','INVARTICLE_CODE','CCODE1','CCODE2','CCODE3','CCODE4','CCODE5','CCODE6','ICODE','GENERATED','MRP','CNAME1','CNAME2','INVHSNSACMAIN_CODE')->where('ICODE', $id)->first();
        // dd($item);
        ...
        $admsite = DB::table('crimsouneclub.admsite')->select('NAME')->where('CODE', $scode)->first();
        
        
        $psite_promo_assign = DB::table('crimsouneclub.psite_promo_assign')->select('STARTDATE','ENDDATE','PROMO_CODE')->where('ADMSITE_CODE', $scode)->where('STATUS', 'A')->get();
        ...
        if(count($psite_promo_assign) > 0) {
            foreach ($psite_promo_assign as $key => $promo_assign) {
                ...
                // if (($today >= $startdate) && ($today <= $enddate)) {
                    // echo 'Start Date :- '.$startdate.' End Date :- '.$enddate.' CODE :- '.$promo_assign->PROMO_CODE.'<br>';
                    $promo_buy = DB::table('crimsouneclub.promo_buy')->select('ASSORTMENT_CODE')->where('PROMO_CODE', $promo_assign->PROMO_CODE)->first();
        ...
        // dd($promo_data);
        $adivision = DB::table('crimsouneclub.promo_assortment_include')
        ...
        $asection = DB::table('crimsouneclub.promo_assortment_include')
                    ->select('ASSORTMENT_CODE')
        ...
        $adepartment = DB::table('crimsouneclub.promo_assortment_include')
                    ->select('ASSORTMENT_CODE')
        ...
        $aarticle = DB::table('crimsouneclub.promo_assortment_include')
                    ->select('ASSORTMENT_CODE')
        ...
        $item_code = DB::table('crimsouneclub.promo_assortment_include')
                    ->select('ASSORTMENT_CODE')
                    ->where('ICODE', $id)
                    ->whereIn('ASSORTMENT_CODE', $promo_data)
                    ->orderBy('ASSORTMENT_CODE','DESC')
        ...
        if (!empty($item->CCODE1)) {
            $category_1 = DB::table('crimsouneclub.promo_assortment_include')
                        ->select('ASSORTMENT_CODE')
        ...
        if (!empty($item->CCODE2)) {
            $category_2 = DB::table('crimsouneclub.promo_assortment_include')
                        ->select('ASSORTMENT_CODE')
                        ->where('CCODE1', $item->CCODE1)
                        ->whereIn('ASSORTMENT_CODE', $promo_data)
                        ->orderBy('ASSORTMENT_CODE','DESC')
        ...
        if (!empty($item->CCODE3)) {
            $category_3 = DB::table('crimsouneclub.promo_assortment_include')
                        ->select('ASSORTMENT_CODE')
                        ->where('CCODE1', $item->CCODE1)
                        ->whereIn('ASSORTMENT_CODE', $promo_data)
                        ->orderBy('ASSORTMENT_CODE','DESC')
        ...
        if (!empty($item->CCODE4)) {
            $category_4 = DB::table('crimsouneclub.promo_assortment_include')
                        ->select('ASSORTMENT_CODE')
                        ->where('CCODE1', $item->CCODE1)
                        ->whereIn('ASSORTMENT_CODE', $promo_data)
                        ->orderBy('ASSORTMENT_CODE','DESC')
        ...
        if (!empty($item->CCODE5)) {
            $category_5 = DB::table('crimsouneclub.promo_assortment_include')
                        ->select('ASSORTMENT_CODE')
                        ->where('CCODE1', $item->CCODE1)
                        ->whereIn('ASSORTMENT_CODE', $promo_data)
                        ->orderBy('ASSORTMENT_CODE','DESC')
        ...
        if (!empty($item->CCODE6)) {
            $category_6 = DB::table('crimsouneclub.promo_assortment_include')
                        ->select('ASSORTMENT_CODE')
                        ->where('CCODE1', $item->CCODE1)
                        ->whereIn('ASSORTMENT_CODE', $promo_data)
                        ->orderBy('ASSORTMENT_CODE','DESC')
        ...
        foreach ($unique_data as $key => $value) {
            ...
            $promobuy = DB::table('crimsouneclub.promo_buy')->select('ASSORTMENT_NAME','PROMO_CODE')->where('ASSORTMENT_CODE', $split_value[0])->whereIn('PROMO_CODE', $psite_promo_list)->first();
        ...
        foreach ($distinct as $key => $value) {
            $final_psite_promo_assign = DB::table('crimsouneclub.psite_promo_assign')->select('PROMO_CODE','STARTDATE','ENDDATE','PRIORITY')->where('PROMO_CODE', $value['PROMO_CODE'])->where('STATUS','A')->where('ADMSITE_CODE', $scode)->first();
            if (count($final_psite_promo_assign) > 0) {
                ...
                if (($today >= $startdate) && ($today <= $enddate)) {
                    $promo_name = DB::table('crimsouneclub.promo_master')->select('NAME')->where('CODE', $final_psite_promo_assign->PROMO_CODE)->first();
        ...
        if (count($final_promo_list) > 0) {
            ...
            foreach ($all_cart['merge'] as $key => $value) {
                foreach ($value as $key => $val) {
                    ...
                    if (array_key_exists('promo_code', $val) && $val['promotion'] != 0) {
                        $is_offer_apply = true;
                        $is_offer = 'Yes';
                        $promo_msg = DB::table('crimsouneclub.promo_master')->where('CODE', $val['promo_code'])->first()->NAME;
            ...
            // $taxmain = DB::table('crimsouneclub.invhsnsacmain as main')
            //     ->select('main.HSN_SAC_CODE', 'det.EFFECTIVE_DATE', 'det.CODE')
            //     ->join('crimsouneclub.invhsnsacdet as det', 'main.CODE', '=', 'det.INVHSNSACMAIN_CODE')
            //     ->where('main.CODE', $item->INVHSNSACMAIN_CODE)
            //     ->first();



            // $taxslab = DB::table('crimsouneclub.invhsnsacslab as slab')
            //         ->select('slab.AMOUNT_FROM', 'slab.INVGSTRATE_CODE','gst.TAX_NAME','gst.CGST_RATE','gst.SGST_RATE','gst.CESS_RATE','gst.IGST_RATE')
            //         ->join('crimsouneclub.invgstrate as gst', 'slab.INVGSTRATE_CODE', 'gst.CODE')
    ...
    public function filter_promotions($promos, $qty, $amt, $scode, $barcode)
    {
        ...
        $promo_m_slab = DB::table('crimsouneclub.promo_slab')
                        ...
                        ->join('crimsouneclub.psite_promo_assign', 'promo_slab.PROMO_CODE', '=', 'psite_promo_assign.PROMO_CODE')
                        ->join('crimsouneclub.promo_master', 'promo_slab.PROMO_CODE', '=', 'promo_master.CODE')
                        ->where('psite_promo_assign.ADMSITE_CODE',$scode)
        ...
        foreach ($final_product as $key => $value) {
            if ($value['is_promo'] == 1) {
                ...
                $promo_slab = DB::table('crimsouneclub.promo_slab')
            ...
            case 'F':
                switch ($value->GET_BENEFIT_CODE) {
                    ...
                    case '2':
                        ...
                        if ($qty == 1) {
                        	// dd($sort_code);
                        	$promo_assortment_include = DB::table('crimsouneclub.promo_assortment_include')->where('ASSORTMENT_CODE', $sort_code[0])->where('CODE', $sort_code[1])->first();
                        	...
                        	if (!$check_cart->isEmpty()) {
                        		foreach ($check_cart as $key => $cart_value) {
                        			// $get_product = DB::table('crimsouneclub.invitem')->where('INVARTICLE_CODE', $promo_assortment_include->INVARTICLE_CODE)->where('ICODE', $id)->first();
                        			// dd($get_product);
                        			if ($cart_value->discount == '0.00' && $cart_value->item_id == $id) {
                                        ...
                                        $getall_barcode = DB::table('crimsouneclub.promo_assortment_include')->select('ICODE')->where('ASSORTMENT_CODE', $sort_code[0])->whereNotNull('ICODE')->get();
                                        $allbarcode = [];
                                        // dd($getall_barcode);
                                        foreach ($getall_barcode as $key => $getall_barcode_value) {
                                            $allbarcode[] = $getall_barcode_value->ICODE;
                                        }
                                        $get_product_from_article = DB::table('crimsouneclub.invitem')->select('ICODE')->where('GRPCODE', $promo_assortment_include->DEPARTMENT_GRPCODE)->get();
    ...
    public function final_cross_check($id, $item, $group, $article, $division, $section, $column, $qid)
    {
        ...
        $total = array();
        $etotal = array();
        $promo_assortment_include = DB::table('crimsouneclub.promo_assortment_include')->where('ASSORTMENT_CODE', $id)->where($column, $qid)->first();
        $promo_assortment_exclude = DB::table('crimsouneclub.promo_assortment_exclude')->where('ASSORTMENT_CODE', $id)->where($column, $qid)->first();
