...
foreach ($versions as $version) {
	...
	$app->group($group, function () use ($app) {
		...
		$app->get('/', function () use ($app) {
			...
			// $request = new Illuminate\Http\Request(['v_id' =>48 , 'client_id' => 'VB4562ASDA34'  , 'ack_id' => '275002K9T0011' ]);
			...
			$invoice->InvoicePush(['v_id' => 48 ,'store_id' =>425,'invoice_id' =>2934,  'client_id' =>2 ]);
		...
		$app->post('/cart-details', ['middleware' => 'settings','uses'	=> 'CartController@cart_details']);
		...
		$app->group(['prefix' => '/vendor'], function () use ($app) {
			...
			$app->group(['middleware' => ['auth', 'vendor_m']], function () use ($app) {
				...
				$app->post('/opening-balance', 'VendorSettlementController@opening_balance');
				...
				$app->post('/table-sync', 'TableSyncController@sync');
		...
		$app->group(['prefix' => '/payment-integration'], function () use ($app) {
			...
			$app->get('/get-transaction', 'PaymentIntegration\PhonePeController@getTransactionById'); 
