...
class StockAdjustmentController extends Controller
{
    ...
    public function posMisPush($params){
     	...
     	$checkExits = OutboundApi::where([ 'for_transaction' => 'Pos Mis Push', 'transaction_id' => $params['adj_id'], 'client_id' => $params['client_id'], 'v_id' => (int)$params['v_id'], 'store_id' => (int)$params['store_id'] ])->first();
    	if(empty($checkExits)) {
    	...
    	try{
	    	...
	    	$outBoundEntryExists = OutboundApi::where([ 'v_id' => $v_id, 'client_id' => $client_id, 'for_transaction' => 'Pos Mis Push', 'transaction_id' => $adj_id ])->first();
	    	...
	    	$outBound->for_transaction = 'Pos Mis Push';
	    	$outBound->transaction_id = $adj_id;
	    	...
	    	$error_for = 'V_id: '.$v_id. ' store_id: '.$store_id.' adj_id: '.$adj_id.' client_id: '.$client_id;

	    	$vendor = Organisation::select('ref_vendor_code')->where('id', $v_id)->first();
			if(!$vendor){

				$error_msg = 'Pos Mis Push Error: '.$error_for.'- Vendor , Message: Unable to find Vendor ';
	    		Log::error( $error_msg );
				...
				$error_msg = 'Pos Mis Push Error: '.$error_for.'- vendor , Message: Unable to find Mapping of vendor ';
	    		Log::error( $error_msg );
			...
			$error_msg = 'Pos Mis Push Error: Error for- '.$error_for.' Message: unexcepted error';
			Log::error($error_msg );
			Log::error($e);
