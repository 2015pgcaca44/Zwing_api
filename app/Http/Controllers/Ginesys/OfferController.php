...
class OfferController extends Controller
{
	...
	public function get_offers(Request $request){
		...
		foreach($vouchers as $voucher){
			...
			$store_credit = [ 'type' => 'voucher' , 'name' => 'VMART' , 'title' => 'store credit' , 'desc' => 'On Time use',  'amount' => $voucherAmount , 'voucher_no' => $voucher->voucher_no , 'voucher_id' => $voucher->id , 'applied_status' => (in_array($voucher->id, $vouchers_ids)?true:false) , 'expired_at' => $expired_at ];
	...
	public function apply_voucher(Request $request){
		...
		if($type == 'voucher'){
			...
			$voucherAmount = $voucher->amount;
			$voucher_applied = DB::table('voucher_applied')->select('applied_amount')->where('voucher_id', $voucher_id)->get();
			...
			if($voucher){
				...
				if($voucher->amount > 0.00){
					...
					DB::table('voucher_applied')->insert(['v_id' => $v_id, 'store_id' => $store_id, 'user_id' => $c_id ,'order_id' => $order_id , 'voucher_id' => $voucher_id, 'applied_amount' => $applied_amount ]);
