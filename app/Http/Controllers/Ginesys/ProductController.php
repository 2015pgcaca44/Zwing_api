...
class ProductController extends Controller {
	...
	public function product_details(Request $request) 
	{
		...
		$params = ['v_id' => $v_id, 'trans_from' => $trans_from, 'barcode' => $barcodefrom, 'qty' =>  $qty, 'mapping_store_id' => $stores->mapping_store_id , 'item' => $item, 'carts' => $carts , 'store_db_name' => $store_db_name, 'is_cart' => 0, 'is_update' => 0, 'c_id' => $c_id ];
		...
		if ($trans_from == 'ANDROID_VENDOR' || $trans_from == 'IOS_VENDOR' || $trans_from == 'CLOUD_TAB' || $trans_from == 'CLOUD_TAB_WEB') {
    ...
    // check product alredy exit cart in ccart
		...
		$check_product_in_cart_exists = $b2bcarts->where('barcode', $barcode)->first();
         ...
         $sizes=DB::table($store_db_name.'.invitem')
            ->select('CNAME4')
            ->distinct()
            ->where('CCODE2', $item->CCODE2)
            ->where('INVARTICLE_CODE', $item->INVARTICLE_CODE)
            ->whereNotNull('CNAME4')
            ->get()->toArray();
            $totals = [];
            $cols = [];
             foreach ($sizes as $key => $size) {
             if($size->CNAME4 != "" && $size->CNAME4 !="NA"){
             	 $totals[] = $size->CNAME4;
             	 $cols[]   = array('label'=>$size->CNAME4,
                                    'qty'  =>0); 
               }
       ...
       if (empty($check_product_in_cart_exists)) {
		$itemlist= [ 
		             ...
		             'SECTION_NAME' => $section->GRPNAME,
		              'DIVISION_NAME' => $division->GRPNAME,
                      'totalqty'    =>0
 		            ];
       
             
        $colors=DB::table($store_db_name.'.invitem')
            ->select('CNAME3')
            ->distinct()
            ->where('CCODE2', $item->CCODE2)
            ->where('INVARTICLE_CODE', $item->INVARTICLE_CODE)
          ...
          foreach ($colors as $key => $color) {
          	if($color->CNAME3 != "" && $size->CNAME4 !="NA"){
             $obj[] =  array('name'=>$color->CNAME3,
                              'size' =>$cols);
             	}
                  
           }                            
      $attribute = array('size' =>$totals,
                         'color' =>$obj);
        }else{
         ...
         $itemlist= [ 
		             ...
		             'SECTION_NAME' => $section->GRPNAME,
		              'DIVISION_NAME' => $division->GRPNAME,
                      'totalqty'    =>$check_product_in_cart_exists->qty,
 		            ];
            $attribute = array('size' =>$totals,
                         'color' =>json_decode($check_product_in_cart_exists->pdata));

        }

		return response()->json(['status' => 'get_product_details', 'message' => 'Get Product Details', 'data' => $itemlist, 'product_image_link' => product_image_link(), 'store_name' => $store_name,'attribute'=>$attribute],  200);
