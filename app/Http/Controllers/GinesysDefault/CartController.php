...
class CartController extends Controller {
	...
	public function add_to_cart(Request $request) {
		...
		$taxs = [];
		// dd($pdata);
		$getvrid = DB::table('ginesys_demo.invitem')->select('ICODE')->where('BARCODE', $barcode)->first();
		if (empty($getvrid)) {
			$barcode = $request->barcode;
		} else {
		...
		$item = DB::table('ginesys_demo.invitem')->select('GRPCODE', 'INVARTICLE_CODE', 'CCODE1', 'CCODE2', 'CCODE3', 'CCODE4', 'CCODE5', 'CCODE6', 'ICODE', 'GENERATED', 'MRP', 'CNAME1', 'CNAME2', 'INVHSNSACMAIN_CODE')->Where('ICODE', $barcode)->first();
		...
		if ($taxmain->SLAB_APPL == 'Y') {
			...
			$onlytax = DB::table('ginesys_demo.invgstrate')->select('TAX_NAME', 'CGST_RATE', 'IGST_RATE', 'SGST_RATE', 'CESS_RATE')->where('CODE', $taxmain->INVGSTRATE_CODE)->first();
	...
	public function order_details(Request $request) {
		...
		foreach ($carts as $key => $value) {
			$applied_offer = [];
			$available_offer = [];
			$item_details = ginesysCategory($value->barcode);
			if ($value->pdata == '""') {
				$offer = 'No';
				$offer_data = (object) [];
			} else {
				...
				$promo_buy = DB::table('ginesys_demo.promo_buy')->select('ASSORTMENT_NAME')->where('PROMO_CODE', $offerdata['promo_code'])->first();
				if ($promo_buy) {
	...
	public function get_carry_bags(Request $request) {
		...
		$carr_bag_arr = ['VR132797', 'VR132799', 'VR132807'];
		$carry_bags = DB::table('ginesys_demo.invitem')->select('ICODE as BAG_ID', 'CNAME1 as Name', 'MRP as Price')->whereIn('ICODE', $carr_bag_arr)->get();
	...
	public function save_carry_bags(Request $request) {
		...
		foreach ($bags as $key => $value) {
			...
			$price_master = DB::table('ginesys_demo.invitem')->select('ICODE as BAG_ID', 'CNAME1 as Name', 'MRP as Price')->where('ICODE', $value[0])->first();
	...
	public function get_print_receipt(Request $request)
    {
        ...
        $site_details = DB::table('ginesys_demo.admsite')->where('CODE', $store->mapping_store_id)->first();
        ...
        foreach ($cart_product as $key => $value) {
            ...
            $getvrid = DB::table('ginesys_demo.invitem')->where('ICODE', $barcode)->first();
            } else {
            $getvrid = DB::table('ginesys_demo.invitem')->where('BARCODE', $barcode)->first();
            }
            //dd($getvrid);
            //
