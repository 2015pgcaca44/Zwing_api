...
class DataPushApiController extends Controller
{
    ...
    public function inbound_api(Request $request)
    {
        ...
        foreach($orders as $order){
            ...
            $orderArr['channelCode'] = 'M11';
            $orderArr['orderLocation'] = $order->store->mapping_store_id;
            ...
            $orderArr['IntegrationUniqueId']= $order->od_id;
            ...
            $orderArr['InvoiceDetail'] = $this->get_data_from_order_details($order);
    ...
    {
        $carts = DB::table('order_details')->select('item_id','qty','unit_mrp','discount','total','tdata')->where(['v_id' => $values->v_id, 'store_id' => $values->store_id, 'user_id' => $values->user_id, 'order_id' => $values->od_id , 'status' => 'success'])->get();
    ...
    public function InvItemNewSync()
    {
        ...
        $local_path = '/home/ginesys_demo/files/item/new/'.$date.'/';
        $ftp_conn = ftp_connect($ftp_server) or die("Could not connect to $ftp_server");
        $login = ftp_login($ftp_conn, 'ginesys_demo', 'v$mart1234');
        $filelist = ftp_nlist($ftp_conn, $path);

        // $custom = DB::connection()->getPdo()->exec('LOAD DATA LOCAL INFILE "/var/www/filedump/ginesys_demo/files/item/invitem-1.csv" INTO TABLE ginesys.invitem FIELDS TERMINATED BY "|"');
        ...
        foreach ($filelist as $key => $value) {
            ...
            if (empty($exists)) {
                $readfilename = 'ftp://ginesys_demo:v$mart1234@'.$ftp_server.'/'.$value;
    ...
    public function InvItemUpdateSync()
    {
        ...
        $local_path = '/home/ginesys_demo/files/item/update/'.$date.'/';
        $ftp_conn = ftp_connect($ftp_server) or die("Could not connect to $ftp_server");
        $login = ftp_login($ftp_conn, 'ginesys_demo', 'v$mart1234');
        $filelist = ftp_nlist($ftp_conn, $path);
    ...
    public function InvArticleNewSync(){
        ...
        $local_path = '/home/ginesys_demo/files/article/new/'.$date.'/';
        $ftp_conn 	= ftp_connect($ftp_server) or die("Could not connect to $ftp_server");
        $login 		= ftp_login($ftp_conn, 'ginesys_demo', 'v$mart1234');
        $filelist 	= ftp_nlist($ftp_conn, $path);
    ...
    public function InvArticleUpdateSync(){
        ...
        $local_path = '/home/ginesys_demo/files/article/update/'.$date.'/';
        $ftp_conn 	= ftp_connect($ftp_server) or die("Could not connect to $ftp_server");
        $login 		= ftp_login($ftp_conn, 'ginesys_demo', 'v$mart1234');
        $filelist 	= ftp_nlist($ftp_conn, $path);
    ...
    public function InvGroupSync(){
        ...
        $local_path = '/home/ginesys_demo/files/group/'.$date.'/';
        $ftp_conn 	= ftp_connect($ftp_server) or die("Could not connect to $ftp_server");
        $login 		= ftp_login($ftp_conn, 'ginesys_demo', 'v$mart1234');
        $filelist 	= ftp_nlist($ftp_conn, $path);
    ...
    public function InvHsnsadetNewSync(){
        ...
        $local_path = '/home/ginesys_demo/files/hsndet/new/'.$date.'/';
        $ftp_conn 	= ftp_connect($ftp_server) or die("Could not connect to $ftp_server");
        $login 		= ftp_login($ftp_conn, 'ginesys_demo', 'v$mart1234');
        $filelist 	= ftp_nlist($ftp_conn, $path);
    ...
    public function InvHsnsadetUpdateSync(){
        ...
        $local_path = '/home/ginesys_demo/files/hsndet/update/'.$date.'/';
        $ftp_conn 	= ftp_connect($ftp_server) or die("Could not connect to $ftp_server");
        $login 		= ftp_login($ftp_conn, 'ginesys_demo', 'v$mart1234');
        $filelist 	= ftp_nlist($ftp_conn, $path);
    ...
    public function InvHsnsacmainNewSync(){
        ...
        $local_path = '/home/ginesys_demo/files/hsnmain/new/'.$date.'/';
        $ftp_conn 	= ftp_connect($ftp_server) or die("Could not connect to $ftp_server");
        $login 		= ftp_login($ftp_conn, 'ginesys_demo', 'v$mart1234');
        $filelist 	= ftp_nlist($ftp_conn, $path);
    ...
    public function InvHsnsacmainUpdateSync(){
        ...
        $local_path = '/home/ginesys_demo/files/hsnmain/update/'.$date.'/';
        $ftp_conn 	= ftp_connect($ftp_server) or die("Could not connect to $ftp_server");
        $login 		= ftp_login($ftp_conn, 'ginesys_demo', 'v$mart1234');
        $filelist 	= ftp_nlist($ftp_conn, $path);
    ...
    public function InvHsnsaclabNewSync(){
        ...
        $local_path = '/home/ginesys_demo/files/hsnslab/new/'.$date.'/';
        $ftp_conn 	= ftp_connect($ftp_server) or die("Could not connect to $ftp_server");
        $login 		= ftp_login($ftp_conn, 'ginesys_demo', 'v$mart1234');
        $filelist 	= ftp_nlist($ftp_conn, $path);
    ...
    public function InvHsnsaclabUpdateSync(){
        ...
        $local_path = '/home/ginesys_demo/files/hsnslab/update/'.$date.'/';
        $ftp_conn 	= ftp_connect($ftp_server) or die("Could not connect to $ftp_server");
        $login 		= ftp_login($ftp_conn, 'ginesys_demo', 'v$mart1234');
        $filelist 	= ftp_nlist($ftp_conn, $path);
    ...
    public function PromoAssortmentSync(){
        ...
        $local_path = '/home/ginesys_demo/files/PromoAssortment/'.$date.'/';
        $ftp_conn 	= ftp_connect($ftp_server) or die("Could not connect to $ftp_server");
        $login 		= ftp_login($ftp_conn, 'ginesys_demo', 'v$mart1234');
        $filelist 	= ftp_nlist($ftp_conn, $path);
    ...
    public function PromoAssortmentExcludeSync(){
        ...
        $local_path = '/home/ginesys_demo/files/PromoAssortment/Exclude/'.$date.'/';
        $ftp_conn 	= ftp_connect($ftp_server) or die("Could not connect to $ftp_server");
        $login 		= ftp_login($ftp_conn, 'ginesys_demo', 'v$mart1234');
        $filelist 	= ftp_nlist($ftp_conn, $path);
    ...
    public function PromoAssortmentIncludeSync(){
        ...
        $local_path = '/home/ginesys_demo/files/PromoAssortment/Include/'.$date.'/';
        $ftp_conn 	= ftp_connect($ftp_server) or die("Could not connect to $ftp_server");
        $login 		= ftp_login($ftp_conn, 'ginesys_demo', 'v$mart1234');
        $filelist 	= ftp_nlist($ftp_conn, $path);
    ...
    public function PromoBuyNewSync(){
        ...
        $local_path = '/home/ginesys_demo/files/PromoBuy/new/'.$date.'/';
        $ftp_conn 	= ftp_connect($ftp_server) or die("Could not connect to $ftp_server");
        $login 		= ftp_login($ftp_conn, 'ginesys_demo', 'v$mart1234');
        $filelist 	= ftp_nlist($ftp_conn, $path);
    ...
    public function PromoBuyUpdateSync(){
        ...
        $local_path = '/home/ginesys_demo/files/PromoBuy/update/'.$date.'/';
        $ftp_conn 	= ftp_connect($ftp_server) or die("Could not connect to $ftp_server");
        $login 		= ftp_login($ftp_conn, 'ginesys_demo', 'v$mart1234');
        $filelist 	= ftp_nlist($ftp_conn, $path);
    ...
    public function PromoMasterNewSync(){
        ...
        $local_path = '/home/ginesys_demo/files/PromoMaster/new/'.$date.'/';
        $ftp_conn 	= ftp_connect($ftp_server) or die("Could not connect to $ftp_server");
        $login 		= ftp_login($ftp_conn, 'ginesys_demo', 'v$mart1234');
        $filelist 	= ftp_nlist($ftp_conn, $path);
    ...
    public function PromoMasterUpdateSync(){
        ...
        $local_path = '/home/ginesys_demo/files/PromoMaster/update/'.$date.'/';
        $ftp_conn 	= ftp_connect($ftp_server) or die("Could not connect to $ftp_server");
        $login 		= ftp_login($ftp_conn, 'ginesys_demo', 'v$mart1234');
        $filelist 	= ftp_nlist($ftp_conn, $path);
    ...
    public function PromoSlabNewSync(){
        ...
        $local_path = '/home/ginesys_demo/files/promoslab/new/'.$date.'/';
        $ftp_conn 	= ftp_connect($ftp_server) or die("Could not connect to $ftp_server");
        $login 		= ftp_login($ftp_conn, 'ginesys_demo', 'v$mart1234');
        $filelist 	= ftp_nlist($ftp_conn, $path);
    ...
    public function PromoSlabUpdateSync(){
        ...
        $local_path = '/home/ginesys_demo/files/promoslab/update/'.$date.'/';
        $ftp_conn 	= ftp_connect($ftp_server) or die("Could not connect to $ftp_server");
        $login 		= ftp_login($ftp_conn, 'ginesys_demo', 'v$mart1234');
        $filelist 	= ftp_nlist($ftp_conn, $path);
    ...
    public function PsitePromoAssignNewSync(){
        ...
        $local_path = '/home/ginesys_demo/files/PromoAssign/new/'.$date.'/';
        $ftp_conn 	= ftp_connect($ftp_server) or die("Could not connect to $ftp_server");
        $login 		= ftp_login($ftp_conn, 'ginesys_demo', 'v$mart1234');
        $filelist 	= ftp_nlist($ftp_conn, $path);
    ...
    public function PsitePromoAssignUpdateSync(){
        ...
        $local_path = '/home/ginesys_demo/files/PromoAssign/update/'.$date.'/';
        $ftp_conn 	= ftp_connect($ftp_server) or die("Could not connect to $ftp_server");
        $login 		= ftp_login($ftp_conn, 'ginesys_demo', 'v$mart1234');
        $filelist 	= ftp_nlist($ftp_conn, $path);
