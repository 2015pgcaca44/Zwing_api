...
class PromotionController extends Controller {
	public function final_check_promo_sitewise($request, $cart = 0) {
		...
		$item = DB::table('ginesys_demo.invitem')->select('GRPCODE', 'INVARTICLE_CODE', 'CCODE1', 'CCODE2', 'CCODE3', 'CCODE4', 'CCODE5', 'CCODE6', 'ICODE', 'GENERATED', 'MRP', 'CNAME1', 'CNAME2', 'INVHSNSACMAIN_CODE', 'STOCKINDATE')->where('ICODE', $id)->first();
		...
		$admsite = DB::table('ginesys_demo.admsite')->select('NAME')->where('CODE', $scode)->first();
		$item_value = [
		...
		// dd($item_value);
		$psite_promo_assign = DB::table('ginesys_demo.psite_promo_assign')->select('STARTDATE', 'ENDDATE', 'PROMO_CODE')->where('ADMSITE_CODE', $scode)->where('STATUS', 'A')->get();
		...
		if (count($psite_promo_assign) > 0) {
			foreach ($psite_promo_assign as $key => $promo_assign) {
				...
				// echo 'Start Date :- '.$startdate.' End Date :- '.$enddate.' CODE :- '.$promo_assign->PROMO_CODE.'<br>';
				$promo_buy = DB::table('ginesys_demo.promo_buy')->select('ASSORTMENT_CODE')->where('PROMO_CODE', $promo_assign->PROMO_CODE)->get();
		...
		foreach ($promo_data as $key => $sort_data) {
			$pai = DB::table('ginesys_demo.promo_assortment_include')->where('ASSORTMENT_CODE', $sort_data['ASSORTMENT_CODE'])->get();
		...
		foreach ($list_assortment as $key => $value) {
			$final_psite_promo_assign = DB::table('ginesys_demo.psite_promo_assign')->select('PROMO_CODE', 'STARTDATE', 'ENDDATE', 'PRIORITY')->where('PROMO_CODE', $value['PROMO_CODE'])->where('STATUS', 'A')->where('ADMSITE_CODE', $scode)->first();
			if (count($final_psite_promo_assign) > 0) {
				...
				if (($today >= $startdate) && ($today <= $enddate)) {
					$promo_name = DB::table('ginesys_demo.promo_master')->select('NAME')->where('CODE', $final_psite_promo_assign->PROMO_CODE)->first();
		...
		if (count($final_promo_list) > 0) {
			...
			foreach ($all_cart['merge'] as $key => $value) {
				foreach ($value as $key => $val) {
					...
					if (array_key_exists('promo_code', $val) && $val['promotion'] != 0) {
						$is_offer_apply = true;
						$is_offer = 'Yes';
						$promo_msg = DB::table('ginesys_demo.promo_master')->where('CODE', $val['promo_code'])->first()->NAME;
			...
			// $taxmain = DB::table('ginesys_demo.invhsnsacmain as main')
			//     ->select('main.HSN_SAC_CODE', 'det.EFFECTIVE_DATE', 'det.CODE')
			//     ->join('ginesys_demo.invhsnsacdet as det', 'main.CODE', '=', 'det.INVHSNSACMAIN_CODE')
			//     ->where('main.CODE', $item->INVHSNSACMAIN_CODE)
			//     ->first();

			// $taxslab = DB::table('ginesys_demo.invhsnsacslab as slab')
			//         ->select('slab.AMOUNT_FROM', 'slab.INVGSTRATE_CODE','gst.TAX_NAME','gst.CGST_RATE','gst.SGST_RATE','gst.CESS_RATE','gst.IGST_RATE')
			//         ->join('ginesys_demo.invgstrate as gst', 'slab.INVGSTRATE_CODE', 'gst.CODE')
	...
	public function filter_promotions($promos, $qty, $amt, $scode, $barcode) {
		...
		$promo_m_slab = DB::table('ginesys_demo.promo_slab')
			...
			->join('ginesys_demo.psite_promo_assign', 'promo_slab.PROMO_CODE', '=', 'psite_promo_assign.PROMO_CODE')
			->join('ginesys_demo.promo_master', 'promo_slab.PROMO_CODE', '=', 'promo_master.CODE')
			->where('psite_promo_assign.ADMSITE_CODE', $scode)
		...
		// $promo_m_slab = DB::table('ginesys_demo.psite_promo_assign')
		...
		//                 ->join('ginesys_demo.promo_slab', 'psite_promo_assign.PROMO_CODE', '=', 'promo_slab.PROMO_CODE')
		//                 ->join('ginesys_demo.promo_master', 'psite_promo_assign.PROMO_CODE', '=', 'promo_master.CODE')
		//                 ->where('psite_promo_assign.ADMSITE_CODE',$scode)
		...
		foreach ($final_product as $key => $value) {
			if ($value['is_promo'] == 1) {
				...
				$promo_slab = DB::table('ginesys_demo.promo_slab')
			...
			case '2':
				...
				if ($qty == $value->SIMPLE_FACTOR) {
					...
					foreach ($sort_code as $pcode) {
						$get_assort = DB::table('ginesys_demo.promo_assortment_include')->where('ASSORTMENT_CODE', $pcode['ASSORTMENT_CODE'])->get();
	...
	public function getAllBarcodeByAssort($array) {
		...
		if ($cloumn_name == 'ICODE') {
			...
			if ($cloumn_name != 'DIVISION_GRPCODE' && $cloumn_name != 'SECTION_GRPCODE' && $cloumn_name != 'DEPARTMENT_GRPCODE') {
				$inbar = DB::table('ginesys_demo.invitem')->where($cloumn_name, end($values))->get(['ICODE'])->toArray();
				return $inbar;
			} elseif ($cloumn_name == 'DEPARTMENT_GRPCODE') {
				$get_art_from_depart = DB::table('ginesys_demo.invarticle')->where('GRPCODE', end($values))->get()->pluck('CODE');
				$inbar = DB::table('ginesys_demo.invitem')->whereIn('INVARTICLE_CODE', $get_art_from_depart)->get(['ICODE'])->toArray();
				// dd($inbar);
				return $inbar;
			}
		}
	...
	public function test_check($item, $acode, $code, $datas) {
		...
		$data = array();
		$return_data = [];
		// $promo_assortment_include = DB::table('ginesys_demo.promo_assortment_include')->where('ASSORTMENT_CODE', $acode['ASSORTMENT_CODE'])->where('CODE', (int)$code)->first();
		// $promo_assortment_exclude = DB::table('ginesys_demo.promo_assortment_exclude')->where('ASSORTMENT_CODE', $acode)->first();
	...
	public function final_cross_check($id, $item, $group, $article, $division, $section, $column, $qid) {
		...
		$total = array();
		$etotal = array();
		$promo_assortment_include = DB::table('ginesys_demo.promo_assortment_include')->where('ASSORTMENT_CODE', $id)->where($column, $qid)->first();
		$promo_assortment_exclude = DB::table('ginesys_demo.promo_assortment_exclude')->where('ASSORTMENT_CODE', $id)->where($column, $qid)->first();
