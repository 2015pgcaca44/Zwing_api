...
class ReturnController extends Controller
{
    ...
    public function get_order(Request $request){
        ...
        if(is_numeric($cust_order)){
            ...
            if($order){
                ...
                //
                return response()->json(['status' => 'success' , 'go_to' => 'order_details' , 'data' => [ 'v_id' => $order->v_id , 'store_id' => $order->store_id , 'c_id' => $order->user_id,  'order_id' => $cust_order, 'trans_from' => $trans_from ]
                 ]);
    ...
    public function get_return_request(Request $request){
        ...
        foreach($carts as $cart){
            ...
            while($loopQty > 0){
               $param[] = $cart->total / $cart->qty; 
               $params[] = ['item_id' => $cart->item_id , 'price' => $cart->total / $cart->qty, 'r_price'=> $cart->subtotal / $cart->qty , 'discount' => $cart->discount / $cart->qty , 'tax' => $cart->tax / $cart->qty  , 'p_name' =>$cart->item_name , 'unit_mrp' => $cart->unit_mrp  , 'cart_id' => $cart->id  , 'bill_discount' => 0];
	...
	public function return_request(Request $request){
        ...
        foreach($return_items as $items ){
        	$cart = DB::table('order_details')->where('id', $items['cart_id'])->first();
	...
	public function approve(Request $request){
        ...
        foreach($return_items as $item ){
            ...
            $tax += $item['tax'];
	       	...
	       	$ex_cart = DB::table('order_details')->where('id', $item['cart_id'])->first();
