...
class CartController extends Controller {
	...
	public function add_to_cart(Request $request) {
		...
		// dd($product_response);
		// $getvrid = DB::table('vmart.invitem')->select('ICODE')->where('BARCODE', $barcode)->first();
		// if (empty($getvrid)) {
		// 	$barcode = $request->barcode;
		// } else {
		...
		// $item = DB::table('vmart.invitem')->select('GRPCODE', 'INVARTICLE_CODE', 'CCODE1', 'CCODE2', 'CCODE3', 'CCODE4', 'CCODE5', 'CCODE6', 'ICODE', 'GENERATED', 'MRP', 'CNAME1', 'CNAME2', 'INVHSNSACMAIN_CODE')->Where('ICODE', $barcode)->first();
	...
	public function order_details(Request $request) {
		...
		foreach ($carts as $key => $value) {
			...
			if ($value->pdata == '""') {
				...
				$promo_buy = DB::table('vmart.promo_buy')->select('ASSORTMENT_NAME')->where('PROMO_CODE', $offerdata['promo_code'])->first();
				if ($promo_buy) {
	...
	public function get_carry_bags(Request $request) {
		...
		$carry_bags = DB::table('vmart.invitem')->select('ICODE as BAG_ID', 'CNAME1 as Name', 'MRP as Price')->whereIn('ICODE', $carr_bag_arr)->get();
	...
	public function save_carry_bags(Request $request) {
		...
		foreach ($bags as $key => $value) {
			...
			$price_master = DB::table('vmart.invitem')->select('ICODE as BAG_ID', 'CNAME1 as Name', 'MRP as Price')->where('ICODE', $value[0])->first();
	...
	public function get_print_receipt(Request $request) {
		...
		$site_details = DB::table('vmart.admsite')->where('CODE', $store->mapping_store_id)->first();
	...
	public function taxCal($barcode, $qty, $mrp, $tax_code)
	{
		...
		$to_amount = 0;
		$tax_name = '';
		$main = DB::table('vmart.invhsnsacmain')->select('HSN_SAC_CODE')->where('CODE', $tax_code)->first();
		$det = DB::table('vmart.invhsnsacdet')->select('CODE','INVGSTRATE_CODE','SLAB_APPL','SLAB_BASIS')->where('INVHSNSACMAIN_CODE', $tax_code)->orderBy('CODE', 'desc')->first();
		...
		if ($det->SLAB_APPL == 'Y') {
			...
			$slabs = DB::table('vmart.invhsnsacslab')->select('INVGSTRATE_CODE', 'AMOUNT_FROM')->where('INVHSNSACMAIN_CODE', $tax_code)->where('INVHSNSACDET_CODE', $det->CODE)->orderBy('AMOUNT_FROM','ASC')->get()->toArray();
			...
			$gst = DB::table('vmart.invgstrate')->where('CODE', $lowest_invgst_rate_code)->first();
			...
			foreach($range as $keyR => $value){
				if($total_taxable_amount >= $value['from'] && $total_taxable_amount < $value['to']){
					...
					$gst = DB::table('vmart.invgstrate')->where('CODE', $invGstRateCode)->first();
					//dd($gst);
