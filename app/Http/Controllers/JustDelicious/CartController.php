...
class CartController extends Controller
{
    ...
    public function product_qty_update(Request $request)
    {
        ...
        return response()->json(['status' => 'product_qty_update', 'message' => 'Product quantity successfully Updated','total_qty' => $check_product_exists->qty , 'total_amount' => $check_product_exists->total], 200);
    ...
    public function process_to_payment(Request $request)
    {
        ...
        foreach ($cart_data as $value) {
            ...
            $res               = DB::table('cart_offers')->where('cart_id',$value['cart_id'])->first();           
            $value['pdata']    = $res->offers;
            /*get tax data - tdata*/
            $tdata    = json_decode($res->offers,true);
            $tdataval = array();
            $tdataval['barcode'] = $tdata['item_id'];
            $tdataval['hsn']     = $tdata['hsn_code'];
            $tdataval['cgst']    = "0";
            $tdataval['sgst']    = "0";
            $tdataval['igst']    = "0";
            $tdataval['cess']    = "0";
            ...
            foreach($tdata['pdata'] as $k => $pdata){
                //dd($pdata['tax']);
                foreach($pdata['tax'] as $tv){
                    if(strpos($tv['tax_desc'],'CGST') !== false ){
                        $tdataval['cgst']    = $tv['tax_rate'];
                        $tdataval['cgstamt'] = $tdataval['cgstamt']+$tv['tax'];
                    }
                    if(strpos($tv['tax_desc'],'SGST') !== false ){
                        $tdataval['sgst']    = $tv['tax_rate'];
                        $tdataval['sgstamt'] = $tdataval['sgstamt']+$tv['tax'];
                    }
                    if(strpos($tv['tax_desc'],'IGST') !== false ){
                        $tdataval['igst']    = $tv['tax_rate'];
                        $tdataval['igstamt'] = $tdataval['igstamt']+$tv['tax'];
                    }
                    if(strpos($tv['tax_desc'],'CESS') !== false){
                        $tdataval['cess']    = $tv['tax_rate'];
                        $tdataval['cessamt'] = $tdataval['cessamt']+$tv['tax'];
                    }
                }
            }
            $tdataval['netamt']  = $tdata['r_price'];
            $tdataval['taxable'] = $tdata['r_price']-$tdata['total_tax'];
            $tdataval['tax']     = $tdata['total_tax'];
            $tdataval['total']   =  $tdataval['taxable']+$tdata['total_tax'];
            $tdataval['tax_name']= "GST";
           /* print_r($tdataval);
            die;*/
            $value['tdata'] = json_encode($tdataval); 
            //$save_order_details = array_except($value, ['cart_id']);
            //$save_order_details = array_add($value, 't_order_id', $porder_id);
            $value['t_order_id'] = $porder_id;
            $save_order_details = $value;
    ...
    public function get_print_receipt(Request $request){
        ...
        foreach ($carts as $key => $cart) {
            ...
            $cart_item_arr[] = ['row' => '1' , 'counter' => $counter , 'hsn_code' => "" , 'cart_item_name' => substr($cart->item_name,0, 20 ) , 'tax_category_name' => '','qty' =>$cart->qty ];
            $kot[]           = ['row' => '1' , 'counter' => $counter , 'hsn_code' => "" , 'cart_item_name' => substr($cart->item_name,0, 20 ) , 'tax_category_name' => '','qty' =>$cart->qty ];
        ...
        foreach($tax_details as $tax_category){
            foreach($tax_category as $tax){
                ...
                $total_tax += $tax['tax'];
                $tax_desc  = explode('_', $tax['tax_desc']);
                $tax_item_arr[] = [ 
                    'tax_desc' => $tax_category_arr[$tax_counter].'  '.$tax_desc[0].'('.$tax['tax_rate'].'%)',
                    ...
                    if($payment->method == 'voucher_credit'){
                        ...
                        foreach($vouchers as $voucher){
                            ...
                            $voucher_applied_list[] = [ 
                                ...
                                'voucher_code' => '' ,
                                 'voucher_amount' => format_number($voucher->amount) ] ;
                        }
        ...
        $paymentMethod = Payment::where('v_id', $order->v_id)->where('store_id',$order->store_id)->where('order_id',$order->order_id)->get()->pluck('method')->all() ;
        //dd($paymentMethod);
        $total_tax = 0;
        ...
        if($voucher_total > 0){
            $zwing_online  =  (float)$zwing_online - (float)$voucher_total;
            $zwing_online = format_number($zwing_online);
        }
       


        $invoices = Invoice::select('invoice_id')->where('ref_order_id', $order->order_id)->where('transaction_type', $order->transaction_type)->first();
        ...
        $print_data_array['store_name'] = $stores->name;
        ...
        //$print_data_array['tax'] = format_number($total_tax);
        //$print_data_array['total_inc_tax'] = format_number($total_inc_tax);

        $print_data_array['tax'] = '';
        $print_data_array['total_inc_tax'] = '';
       ...
       // $print_data_array['thank_you_msg'] = 'THANK YOU !!! DO VISIT AGAIN'.'\n'.'E&OE'.'\n'.'FOR EXCHANGE POLICY'.'\n'.'PLEASE SEE THE REVERSE OF THE BILL'.'\n'.'GENERATED BY ZWING';
        $print_data_array['thank_you_msg']  = 'GENERATED BY ZWING';
        ...
        $print_data_array['kot'] = $kot;


        $response = ['status' => 'success' , 'data' => $print_data_array ];
...
*/
        ...
        <html>
            ...
            <body>
                ...
                <center>
                    <div> 
                    <img style="float:left; height:30px" src="'.$bilLogo.'" >
        ...
        foreach ($carts as $key => $cart) {
            ...
            foreach ($final_data['pdata'] as $key => $value) {
                ...
                foreach ($tax_rates as $tkey => $tax_rate) {
                    ...
                    $taxable_amount = round ( ($value['ex_price'] / $value['qty']) , 2 );
