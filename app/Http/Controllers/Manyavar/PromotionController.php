...
class PromotionController extends Controller {
	public function final_check_promo_sitewise($request, $cart = 0) {
		...
		$item = DB::table('manyavar.invitem')->select('GRPCODE', 'INVARTICLE_CODE', 'CCODE1', 'CCODE2', 'CCODE3', 'CCODE4', 'CCODE5', 'CCODE6', 'ICODE', 'GENERATED', 'MRP', 'CNAME1', 'CNAME2', 'INVHSNSACMAIN_CODE', 'STOCKINDATE')->where('ICODE', $id)->first();
		...
		if (count($psite_promo_assign) > 0) {
			foreach ($psite_promo_assign as $key => $promo_assign) {
				...
				// echo 'Start Date :- '.$startdate.' End Date :- '.$enddate.' CODE :- '.$promo_assign->PROMO_CODE.'<br>';
				$promo_buy = DB::table('manyavar.promo_buy')->select('ASSORTMENT_CODE')->where('PROMO_CODE', $promo_assign->PROMO_CODE)->get();
		...
		foreach ($promo_data as $key => $sort_data) {
			$pai = DB::table('manyavar.promo_assortment_include')->where('ASSORTMENT_CODE', $sort_data['ASSORTMENT_CODE'])->get();
	...
	public function filter_promotions($promos, $qty, $amt, $scode, $barcode) {
		...
		// $promo_m_slab = DB::table('manyavar.psite_promo_assign')
		...
		//                 ->join('manyavar.promo_slab', 'psite_promo_assign.PROMO_CODE', '=', 'promo_slab.PROMO_CODE')
		//                 ->join('manyavar.promo_master', 'psite_promo_assign.PROMO_CODE', '=', 'promo_master.CODE')
		//                 ->where('psite_promo_assign.ADMSITE_CODE',$scode)
			...
			case '2':
				...
				if ($qty == $value->SIMPLE_FACTOR) {
					...
					foreach ($sort_code as $pcode) {
						$get_assort = DB::table('manyavar.promo_assortment_include')->where('ASSORTMENT_CODE', $pcode['ASSORTMENT_CODE'])->get();
	...
	public function getAllBarcodeByAssort($array) {
		...
		if ($cloumn_name == 'ICODE') {
			...
			if ($cloumn_name != 'DIVISION_GRPCODE' && $cloumn_name != 'SECTION_GRPCODE' && $cloumn_name != 'DEPARTMENT_GRPCODE') {
				$inbar = DB::table('manyavar.invitem')->where($cloumn_name, end($values))->get(['ICODE'])->toArray();
				return $inbar;
			} elseif ($cloumn_name == 'DEPARTMENT_GRPCODE') {
				$get_art_from_depart = DB::table('manyavar.invarticle')->where('GRPCODE', end($values))->get()->pluck('CODE');
				$inbar = DB::table('manyavar.invitem')->whereIn('INVARTICLE_CODE', $get_art_from_depart)->get(['ICODE'])->toArray();
				// dd($inbar);
				return $inbar;
			}
		}
	...
	public function test_check($item, $acode, $code, $datas) {
		...
		$data = array();
		$return_data = [];
		// $promo_assortment_include = DB::table('manyavar.promo_assortment_include')->where('ASSORTMENT_CODE', $acode['ASSORTMENT_CODE'])->where('CODE', (int)$code)->first();
		// $promo_assortment_exclude = DB::table('manyavar.promo_assortment_exclude')->where('ASSORTMENT_CODE', $acode)->first();
