...
class CartController extends Controller
{
    ...
    public function cart_details(Request $request)
    {
            ...
            foreach ($carts as $key => $cart) {
                ...
                $res = DB::table('Cart')->where('cart_id',$cart->cart_id)->first();
        ...
        if($total == 0.00){
            ...
            try{
                ...
                foreach($carts as $cart){
                    ...
                    //dd($cartId);
                    $tempCartDetails = DB::table('Cart_details')
                    ...
                    $tempCartOffers = DB::table('Cart_offers')
                    ...
                    DB::table('Cart')->where('cart_id', $tempCartId)->delete();
                    DB::table('Cart_details')->where('cart_id',  $tempCartId)->delete();
                    DB::table('Cart_offers')->where('cart_id',  $tempCartId)->delete();
                }
    ...
    public function order_details(Request $request)
    {
        ...
        $carts = OrderDetails::where('t_order_id', $order_num_id->od_id)->where('v_id', $v_id)->where('store_id', $store_id)->where('user_id', $c_id)->get();
        ...
        if($order_num_id->transaction_type == 'sales'){
            ...
            $return_order = DB::table('orders as o')
                            ...
                            ->where('o.ref_order_id' , $order_num_id->order_id)
                            ->where('o.transaction_type','return')
                            //->select('od.*')
                             ->selectRaw('sum(qty) as sum, od.*')
                            ->get();

            $return_item_ids = $return_order->pluck('sum','item_id')->all();
        }
        foreach ($carts as $key => $cart) {
            $res = DB::table('order_details')->where('t_order_id',$cart['t_order_id'])->first();
            $offer_data = json_decode($res->pdata, true);
            foreach ($offer_data['pdata'] as $key => $value) {
            ...
            if (isset($return_item_ids[$cart->item_id]) ) {
                $return_product_qty = $cart->qty - $return_item_ids[$cart->item_id];
            }
            $product_data['return_flag'] = $return_flag;
            $product_data['return_qty'] = (string)$return_product_qty;
            $product_data['carry_bag_flag'] = $carry_bag_flag;
  ...
  //   }
    ...
    public function get_print_receipt(Request $request){
        ...
        $carts = OrderDetails::where('user_id', $c_id)->where('v_id', $v_id)->where('store_id', $store_id)->where('order_id', $order->o_id)->get();
        ...
        foreach ($carts as $key => $cart) {
            ...
            $total_amount = DB::table('order_details')->where('t_order_id', $order_details->od_id)->where('v_id', $order_details->v_id)->where('store_id', $order_details->store_id)->where('user_id', $order_details->user_id)->sum('total');
            $cart_product = DB::table('order_details')->where('t_order_id', $order_details->od_id)->where('v_id', $order_details->v_id)->where('store_id', $order_details->store_id)->where('user_id', $order_details->user_id)->get();
            //print_r($cart_product);exit();
            // $offer_data = $cart_product->pluck('pdata');

            foreach ($cart_product as $key => $cartItem) {
         
                $item_master = DB::table($store_db_name.'.item_master')->where('ITEM',$cartItem->item_id)->first();
                // $res = DB::table('order_details')->where('cart_id',$cart->cart_id)->first();
                ...
                $offer_data = json_decode($cartItem->pdata, true);
                //dd($offer_data['pdata']);
                foreach ($offer_data['pdata'] as $key => $value) {
                    $tax_details_data[$cart->item_id] = ['tax' =>  $value['tax'] , 'total' => $value['ex_price'] ];
	...
	public function order_receipt($c_id,$v_id , $store_id, $order_id){
        ...
        foreach ($carts as $key => $cart) {
			...
			if($order->transaction_type == 'sales'){
                ...
                $res = DB::table('order_details')->where('t_order_id',$orderData->od_id)->first();
                $offer_data = json_decode($res->pdata, true);
                ...
                $ref_cart = OrderDetails::select('t_order_id')->where('v_id',$v_id)->where('store_id',$store_id)->where('user_id',$c_id)->where('order_id' , $ref_order->o_id)->where('item_id' , $cart->item_id)->first();
                $orderData = DB::table('orders')->where('order_id',$order_id)->first();
                $res = DB::table('order_details')->where('t_order_id',$orderData->od_id)->first();
                $offer_data = json_decode($res->pdata, true);
            }
