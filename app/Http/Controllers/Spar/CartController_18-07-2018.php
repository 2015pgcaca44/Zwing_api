...
class CartController extends Controller
{
	...
	public function add_to_cart(Request $request)
    {
        ...
        $pdata = $request->pdata;
        $data = json_decode(urldecode($pdata));
		//dd($data);
    ...
    public function apply_employee_discount(Request $request){
        ...
        //dd($employee_details);
        if($employee_details){
            $employee = DB::table($store_db_name.'.employee_details')->where('employee_id', $employee_details->Employee_ID)->first();
            
            if($employee){

                 DB::table($store_db_name.'.employee_details')->update(['available_discount' => $employee_details->Available_Discount_Amount]);
                
            }else{
                DB::table($store_db_name.'.employee_details')->insert([
            ...
            $params = ['v_id' => $v_id , 'store_id' => $store_id , 'c_id' => $c_id , 'employee_available_discount' => $employee_details->Available_Discount_Amount , 'employee_id' => $employee_details->Employee_ID ];
    ...
    public function cart_details(Request $request)
    {
            ...
            $carts = Cart::where('user_id', $c_id)->where('v_id', $v_id)->where('store_id', $store_id)->where('status','process')->orderBy('cart_id','desc')->get();
            ...
            if($settings){
                ...
                if(isset($productSettings->max_qty)){
                    $product_max_qty = $productSettings->max_qty;    
                }
    ...
    public function process_to_payment(Request $request)
    {
        ...
        if($total == 0.00){
            ...
            if($ord->employee_discount > 0.00){
                ...
                $emp_det = DB::table($store_db_name.'.employee_details')->where('employee_id', $ord->employee_id)->first();
        ...
        $file = fopen($path_with_file_name,"w");
         ...
         foreach ($orders as $key => $order) {
            ...
            foreach ($carts as $key => $cart) {
                ...
                $tic_no = '';
                if($cust){
                    $cust_d = DB::table($store_db_name.'.customer_details')->select('tic_no')->where('mobile', $cust->mobile)->first();
        ...
        foreach ($orders as $key => $order) {
            ...
            foreach ($carts as $key => $cart) {
                ...
                $items = [
                    ...
                    $cart->unit_mrp,
                    ($cart->weight_flag)?'KG':'EA', //EA and KG
                    $order->return_code, //REturn REAson cdoe
                    $promo_type , //Promo Type
                    $discount_type, //Discount Type
                    $discount
                ];
    ...
    public function process_each_item_in_cart($param){
        ...
        if(isset($param['employee_available_discount'])){
            ...
            $emp_details = DB::table($store_db_name.'.employee_details')->where('employee_id', $employee_id)->first();
