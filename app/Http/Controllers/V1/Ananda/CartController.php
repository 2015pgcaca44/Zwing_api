...
class CartController extends Controller {
	...
	public function add_to_cart(Request $request) {
		...
		if (@$taxmain->SLAB_APPL == 'Y') {
			...
			$onlytax = DB::table('ananda.invgstrate')->select('TAX_NAME', 'CGST_RATE', 'IGST_RATE', 'SGST_RATE', 'CESS_RATE')->where('CODE', $taxmain->INVGSTRATE_CODE)->first();
			...
			$onlytax = DB::table('ananda.invgstrate')->select('TAX_NAME', 'CGST_RATE', 'IGST_RATE', 'SGST_RATE', 'CESS_RATE')->where('CODE', '0')->first();
			$taxs['apply_tax'] = $onlytax;
			$dividegst = 100 + $onlytax->IGST_RATE;
			$actual_price = $s_price * 100;
			$actual_price = $actual_price / $dividegst;
	...
	public function product_qty_update(Request $request) {
		...
		$cartPost = (object) ['v_id' => $v_id, 'store_id' => $store_id, 'c_id' => $c_id, 'qty' => $qty, 'barcode' => $barcode, 'unit_mrp' => $returnProduct['unit_mrp'], 'r_price' => $returnProduct['r_price'], 's_price' => $returnProduct['s_price'], 'discount' => $returnProduct['discount'], 'pdata' => $returnProduct['pdata'], 'data' => $returnProduct];
		...
		foreach ($check_product_exists as $key => $delete_update) {
			...
			// dd($response_data);
			$add_to_cart_reponse = (object) ['v_id' => $v_id, 'c_id' => $c_id, 'store_id' => $store_id, 'barcode' => $response_data['p_id'], 'qty' => $response_data['qty'], 'unit_mrp' => $response_data['unit_mrp'], 'r_price' => $response_data['r_price'], 's_price' => $response_data['s_price'], 'discount' => $response_data['discount'], 'pdata' => $response_data['pdata'], 'data' => $response_data];
	...
	public function cart_details(Request $request) {
		...
		foreach ($vouchers as $key => $voucher) {
			array_push($voucher_array, ['name' => 'ananda Credit', 'amount' => $voucher->amount]);
	...
	public function process_to_payment(Request $request) {
		...
		if ($pay_by_voucher > 0.00 && $total == 0.00) {
			...
			$request->request->add(['t_order_id' => $t_order_id, 'order_id' => $order_id, 'pay_id' => 'user_order_id_' . $t_order_id, 'method' => 'ananda_credit', 'invoice_id' => '', 'bank' => '', 'wallet' => '', 'vpa' => '', 'error_description' => '', 'status' => 'success', 'payment_gateway_type' => 'Voucher', 'cash_collected' => '', 'cash_return' => '', 'amount' => $pay_by_voucher]);
	...
	public function process_each_item_in_cart($param) {
		...
		foreach ($check_product_exists as $key => $delete_update) {
			...
			$response_data = $product->product_details_by_qty($request_data);
			// dd($response_data);
			$add_to_cart_reponse = (object) ['v_id' => $param['v_id'], 'c_id' => $param['c_id'], 'store_id' => $param['store_id'], 'barcode' => $response_data['p_id'], 'qty' => $response_data['qty'], 'unit_mrp' => $response_data['unit_mrp'], 'r_price' => $response_data['r_price'], 's_price' => $response_data['s_price'], 'discount' => $response_data['discount'], 'pdata' => $response_data['pdata'], 'data' => $response_data];
			$this->auto_add_to_cart($add_to_cart_reponse);
			// dd($add_to_cart_reponse);
		}
		// dd($check_product_exists);
	...
	public function get_print_receipt(Request $request) {
		...
		if ($order_details->transaction_type == 'sales') {
			...
			if ($payments) {
				...
				foreach ($payments as $payment) {
					...
					if ($payment->method == 'ananda_credit') {
						$vouchers = DB::table('voucher_applied as va')
							->join('voucher as v', 'v.id', 'va.voucher_id')
		...
		$data = [
			...
			'time' => date('d-M-Y h:i A', strtotime($order_details->created_at)),
			...
			'due' => $total_amount,
			'in_words' => "Rs ".numberTowords(round($order_details->total))." Only.",
