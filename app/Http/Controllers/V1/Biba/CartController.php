...
class CartController extends Controller
{
	...
	public function add_to_cart(Request $request)
    {
           ...
           // $plu_qty = substr($barcode,7, 5);
            ...
            if($weight_flag){
                ...
                if(count($cart_plu_qty) > 1 ){
                    ...
                    $check_product_exists = TempCart::where('store_id', $store_id)->where('v_id', $v_id)->where('order_id', $order_id)->where('user_id', $c_id)->where('plu_barcode', $plu_barcode)->where('status', 'process')->count();    
                }
            ...
            $check_product_exists = TempCart::where('store_id', $store_id)->where('v_id', $v_id)->where('order_id', $order_id)->where('user_id', $c_id)->where('barcode', $barcode)->where('status', 'process')->count();
        ...
        if($temp){
            DB::table('temp_cart_offers')->insert([
        ...
        if($request->has('process_each_item') && $request->process_each_item != 1){
            $carts = TempCart::where('user_id', $c_id)->where('v_id', $v_id)->where('store_id', $store_id)->where('status','process')->get();
        ...
        $response = ['status' => 'add_to_cart', 'message' => 'Product was successfully added to your cart.',
        //'data' => $cart ,
    ...
    public function product_qty_update(Request $request)
    {
        ...
        $check_product_exists = TempCart::where('store_id', $store_id)->where('v_id', $v_id)->where('order_id', $order_id)->where('user_id', $c_id)->where('barcode', $barcode)->where('status', 'process')->first();
        ...
        return response()->json(['status' => 'product_qty_update', 'message' => 'Product quantity successfully Updated' , 'total_qty' => $cart_summa['total_qty'] , 'total_amount' => $cart_summa['total_amount'] ], 200);
    ...
    public function remove_product(Request $request)
    {
        ...
        if($request->has('all')){
            if($request->all == 1){
                ...
                $carts = TempCart::where('store_id', $store_id)->where('v_id', $v_id)->where('order_id', $order_id)->where('user_id', $c_id)->where('status', 'process')->get();

                foreach ($carts as $key => $cart) {
                    TempCart::where('cart_id', $cart->cart_id)->delete();
            ...
            if($request->has('cart_id')){
                ...
                TempCart::where('cart_id', $cart_id)->delete();
                DB::table('temp_cart_details')->where('cart_id' , $cart_id)->delete();
                DB::table('temp_cart_offers')->where('cart_id' , $cart_id)->delete();
            }
    ...
    public function cart_details(Request $request)
    {
            ...
            $carts = TempCart::where('user_id', $c_id)->where('v_id', $v_id)->where('store_id', $store_id)->where('status','process')->orderBy('updated_at','desc')->get();
            ...
            foreach ($carts as $key => $cart) {
                ...
                if($cart->override_flag == '1'){
                    $product_data['s_price'] = format_number($cart->override_unit_price * $cart->qty );
                }else{
            ...
            if(!empty($bill_buster_dis)){
                ...
                $bill_buster_id = DB::table('temp_table')->insertGetId(['bill_buster' => json_encode($bill_buster_dis) ]);
            }
            ...
            foreach ($vouchers as $key => $voucher) {
                array_push($voucher_array ,['name' => 'Spar Credit' , 'amount' => $voucher->amount ] );
    ...
    public function process_to_payment(Request $request)
    {
        ...
        $bill_buster_re ='';
        $bill_buster_id = 0;
        if($request->has('bill_buster_id')){
            $bill_buster_id = $request->bill_buster_id;
            if($bill_buster_id > 0 ){
                $billda = DB::table('temp_table')->where('id', $bill_buster_id)->first();
                if($billda){
                    $bill_buster_re = $billda->bill_buster;
                }
            }
        }
        ...
        $order->bill_buster_data = $bill_buster_re;
        $order->tax = $tax;
        ...
        if($bill_buster_id > 0 ){
            DB::table('temp_table')->where('id', $bill_buster_id)->delete();
        }
        ...
        if($total == 0.00){
            ...
            try{
                ...
                TempCart::where('store_id', $store_id)->where('v_id', $v_id)->where('order_id', $ord->o_id)->where('user_id', $c_id)->update(['status' => 'success']);
                
                $carts = TempCart::where('store_id', $store_id)->where('v_id', $v_id)->where('order_id', $ord->o_id)->where('user_id', $c_id)->get();
                //dd($ord);
    ...
    public function payment_details(Request $request)
    {
        ...
        if($payment_gateway_type =='RAZOR_PAY'){
            ...
            if($status == 'success'){
                ...
                $ord = Order::where('store_id', $store_id)->where('v_id', $v_id)->where('order_id', $order_id)->first();
                $ord->status = $status;
                $ord->error_description = $error_description;
                $ord->save();
            }
        ...
        if($payment_save_status){
            ...
            try{
                ...
                TempCart::where('store_id', $store_id)->where('v_id', $v_id)->where('order_id', $ord->o_id)->where('user_id', $user_id)->update(['status' => $status]);
                
                $carts = TempCart::where('store_id', $store_id)->where('v_id', $v_id)->where('order_id', $ord->o_id)->where('user_id', $user_id)->get();
                //dd($ord);
                ...
                foreach($carts as $cart){
                    ...
                    $cartArray['t_order_id'] = $ord->order_id;
            ...
            foreach($carts as $cart){
                ...
                //dd($cart->toArray());
                $cartArray = $cart->toArray();
                $cartArray['t_order_id'] = $order_id;
                $cartArray['status'] = $status;
                $cartId = Cart::insertGetId($cartArray);
                //dd($cartId);
                $tempCartDetails = DB::table('temp_cart_details')
                //$text = " $cartId as cart_id";
            ...
            return response()->json(['status' => 'fail' , 'message' => 'Your order has been failed you can retry again' ] );
        }
    }
    ...
    public function order_details(Request $request)
    {
        ...
        $carts = Cart::where('user_id', $c_id)->where('v_id', $v_id)->where('store_id', $store_id)->where('order_id', $o_id->o_id);
        if($o_id->status == 'error'){
           $carts = $carts ->where('t_order_id', $o_id->order_id) ;
        }else{
            $carts = $carts ->where('status', $o_id->status) ;
        }
        $carts = $carts->get();
        $sub_total = $carts->sum('subtotal');
        ...
        foreach ($carts as $key => $cart) {
            ...
            $productImage = new VendorImageController();
            $item_image   = $productImage->getItemImage($cart->barcode,$stores->store_db_name);

            $product_data['varient'] = '';
            $product_data['images'] = $item_image;
            $product_data['description'] = '';
            $product_data['deparment'] = '';
            $product_data['barcode'] = $cart->barcode;
        ...
        $order_header_img   = $vendorImage->getImage($o_id->v_id,$vendorImage->Detail_Header_Logo);
        ...
        return response()->json(['status' => 'order_details', 'message' => 'Order Details Details', 
            ...
            'product_image_link' => product_image_link().$o_id->v_id,
            'store_header_logo' => env('ADMIN_URL').$order_header_img,
            //'offer_data' => $global_offer_data,
    ...
    public function get_print_receipt(Request $request){
        ...
        /*$bilLogo = '';
        $vendorImage  = VendorImage::where('v_id', $v_id)->where('type', 5)->where('status',1)->first();
        if($vendorImage)
        {
        ...
        $print_data_array['bill_logo'] = $bilLogo;
	...
	public function order_receipt($c_id,$v_id , $store_id, $order_id){
        ...
        <html>
            ...
            <body>
                ...
                <center>
                    <div> 
                    <img style="float:left" src="'.$bilLogo.'" >
    ...
    public function rt_log(Request $request){
		//try {
            ...
            if(env('APP_ENV') == 'staging'){
                $rtlogFolder = 'rtlogs_staging';
            }
            if(env('APP_ENV') == 'production'){
                $rtlogFolder = 'rtlogs';
            }
			$path_with_file_name = '/home/sparut/ftp/'.$rtlogFolder.'/'.$stores->mapping_store_id
			.'/'.$file_name;
			...
			foreach ($orders as $key => $order) {
                ...
                $newItems=[];
                //Condition added for recalculation tax, and item discount and item total when bill buster applied
                $param=[];
                $params=[];
                $tax_details = [];
                $bill_discount = 0;
                ...
                if($bill_buster_discount > 0 ){
                    ...
                    foreach($params as $key => $para){
                        if( isset($newItems[$para['item_id']]) ){
                            $newItems[$para['item_id']]['discount'] += $para['discount'];
                            $newItems[$para['item_id']]['price'] += $para['price'];
                            $newItems[$para['item_id']]['qty'] += 1;
                        }else{
                            $newItems[$para['item_id']]['item_id'] = $para['item_id'];
                            $newItems[$para['item_id']]['discount'] = $para['discount'];
                            $newItems[$para['item_id']]['price'] = $para['price'];
                            $newItems[$para['item_id']]['qty'] = 1;
                        }
                    ...
                    foreach($newItems as $key => $para){
                        ...
                        $newItems[$key]['tax'] = $tax_details;
                        $newItems[$key]['total'] = $newItems[$key]['price'] - $newItems[$key]['discount'];
                    }
				...
				foreach ($carts as $key => $cart) {
                    ...
                    if(count($newItems)== 0 ){
                        ...
                        $item_total = $newItems[$cart->item_id]['total'];
                        $total += $item_total;
                        $total += $newItems[$cart->item_id]['total'];
                        $tax_details = $newItems[$cart->item_id]['tax'];
                        $bill_discount = $newItems[$cart->item_id]['discount'];
                    ...
                    if($cart->override_flag == '1'){
                        $unit_retail = $cart->override_unit_price;
                    }else{
                        $unit_retail = $cart->total / $cart->qty;
                    }
					...
					$items[$items_index++] = $item_total;//Amount
			...
			#### This entry for Return Items START #########
			...
			foreach ($orders as $key => $order) {
				...
				foreach ($carts as $key => $cart) {
					...
					$promo_type = '1004';
					$discount_type = 'ORRCAP';
					$discount = $cart->discount + $cart->bill_buster_discount;
					if($cart->employee_discount > 0.00){
						$promo_type = '10044';
						$discount_type = 'ORRCAPP';
						$discount = $cart->employee_discount;
		...
		/*} catch(\Exception $e){
            ...
            Mail::raw( $errorMessage, function ($message) {
               $message->to('chandramani.s@gsl.in')
                        ->subject('Rt-log Error')
                        ->cc('rishabh.s@gsl.in')
                        ;
            });

            return ['status' => 'fail', 'message' => 'Error has Occured: '.$errorMessage];
        }*/
    ...
    public function get_carry_bags(Request $request)
    {
        ...
        $carr_bag_arr =  [ '100002519', '100003551'];
    ...
    public function process_each_item_in_cart($param){
        ...
        $cartss = TempCart::where('store_id', $store_id)->where('v_id', $v_id)->where('order_id', $order_id)->where('user_id', $c_id)->where('status', 'process')->get();
        ...
        return ['total_qty' => (string)$carts->sum('qty') , 'total_amount' => format_number($carts->sum('total')) ] ; 
