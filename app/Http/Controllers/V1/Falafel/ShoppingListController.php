...
namespace App\Http\Controllers\V1\Falafel;
...
class ShoppingListController extends Controller
{
    ...
    public function list(Request $request){
        ...
        $shoppingList = DB::table('shopping_list as sl')
                        ->join($store_db_name.'.price_master' , 'price_master.ITEM' ,'sl.item_id')
                        ->select('sl.id','sl.barcode','sl.qty','sl.amount','sl.for_date','sl.added_to_cart','price_master.ITEM_DESC as product_name')
    ...
    public function create(Request $request)
    {
    	...
    	$check_product_exists = ShoppingList::where('store_id', $store_id)->where('v_id', $v_id)->where('user_id', $c_id)->where('barcode', $barcode)->where('for_date', $for_date)->count();
    	...
    	$shoppingList->item_id = $product_id;
    ...
    public function product_qty_update(Request $request){
    	...
    	$check_product_exists = ShoppingList::where('store_id', $store_id)->where('v_id', $v_id)->where('user_id', $c_id)->where('barcode', $barcode)->where('for_date', $for_date)->first();
