...
class CartController extends Controller
{
	...
	public function add_to_cart(Request $request)
    {
        ...
        $price_master = DB::table($store_db_name.'.price_master')->where('ITEM', $item_master->ITEM)->select('ITEM_DESC')->first();
    ...
    {   
        ...
        $price_flag = $vendorS->getPricePriceSetting(['v_id' => $v_id , 'trans_from' => $trans_from]);
        $price_flag_status = $price_flag->status;
        ...
        $request->request->add(['process_each_item' => 0]);
        ...
        foreach ($items as $key => $value) {
            ...
            if($exists) {
                ...
                if($value[1] < 1 ){
                    ...
                    $unit_mrp = 0;
                    $qty = $value[1] + $exists->qty;
                    if($price_flag_status == 1){
                        $unit_mrp = $price_master->MRP1;
                    }
                    $request->request->add(['barcode' => $value[0] , 'qty' =>$qty , 'unit_mrp' =>  $unit_mrp , 'r_price' => $unit_mrp * $qty , 's_price' => $unit_mrp * $qty , 'discount' => 0 , 'pdata' => '' ]);
                ...
                if($value[1] > 0 ){
                    ...
                    $request->request->add(['barcode' => $value[0] , 'qty' =>$value[1] , 'unit_mrp' => $unit_mrp  , 'r_price' => $unit_mrp * $value[1] , 's_price' => $unit_mrp * $value[1] , 'discount' => 0 , 'pdata' => '' ]);
        ...
        if($status == 1) {
            return response()->json(['status' => 'save_cart', 'message' => 'Catalogs Added' , 'total' => format_number($total) ],200);
        } else {
            return response()->json(['status' => 'save_cart', 'message' => 'Catalogs  Updated' ,'total' => format_number($total) ],200);
        }
    }
    ...
    public function remove_product(Request $request)
    {
    	...
    	return response()->json(['status' => 'remove_product', 'message' => 'Item Removed successfully' , 'total_qty' => $cart_summa['total_qty'] , 'total_amount' => $cart_summa['total_amount'] ],200);
    ...
    public function cart_details(Request $request)
    {
            ...
            foreach ($carts as $key => $cart) {
                ...
                if(isset($res->offers)){
                    ...
                    $offer_data = ['pdata' => [] , 'available_offer' => [] , 'applied_offer' => [] , 'multiple_price_flag' => false , 'r_price' => 0 , 's_price' =>  0];
                }
            ...
            foreach ($vouchers as $key => $voucher) {
                array_push($voucher_array ,['name' => 'Metro Credit' , 'amount' => $voucher->amount ] );
    ...
    public function process_to_payment(Request $request)
    {
        ...
        if($total == 0.00){
            ...
            return response()->json(['status' => 'payment_save', 'go_to' => $go_to ,'bill_logo' => $bilLogo,'message' => 'Save Payment', 'data' => $payment , 'redirect_to_qr' => true ],200);
            exit;
    ...
    public function payment_details(Request $request)
    {
        ...
        if($payment_save_status){
            ...
            return response()->json(['status' => 'payment_save', 'go_to' => $go_to , 'message' => 'Save Payment', 'data' => $payment ],200);
        }
    }
    ...
    public function order_qr_code(Request $request)
    {
        ...
        if( $request->has('go_to')){
            if($request->go_to == 'pos_checkout'){
                $order = DB::table('orders')->where('order_id', $order_id)->select('v_id','store_id','user_id','o_od')->first();
    ...
    public function get_print_receipt(Request $request){
        ...
        foreach ($carts as $key => $cart) {
            ...
            $cart_item_arr[] = ['row' => '1' , 'counter' => $counter , 'hsn_code' => $hsn_code , 'cart_item_name' => str_replace('\n',' ',substr($cart->item_name,0, 20 ) ), 'tax_category_name' => $cart_tax_code_msg ];
        ...
        $print_data_array['store_name'] = 'METRO INDIA PVT LTD';
	...
	public function order_receipt($c_id,$v_id , $store_id, $order_id){
        ...
        <html>
            ...
            <body>
                ...
                <center>
                    <div> 
                    <img style="float:left" src="'.store_logo_link().'metro_logo.png" >
					<img style="float:right;height:30px" src="'.env('APP_URL').'/images/zwing_header.png" >

					</div>
					<div class="clearfix"></div>
                    <p>Metro INDIA PVT LTD</P>
                    <hr/>
