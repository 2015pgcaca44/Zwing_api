	...
	public function my_order(Request $request)
	{
		...
		if(Auth::user()->vendor_id == 1){
			...
			if($request->has('sort')){
				...
				$orders = $orders->orderBy('o.od_id', $sort); // value of sort desc or asc
			}else{
			...
			$orders->select('c.first_name','c.last_name','c.mobile','o.od_id','o.transaction_type','o.user_id','o.order_id','o.total','o.date','o.status','o.v_id','o.store_id','o.o_id');
			...
			foreach ($orders as $key => $value) {
				$carts = Cart::where('v_id', $value->v_id)->where('store_id', $value->store_id)->where('user_id', $value->user_id)->where('order_id', $value->o_id)->get();
				...
				$store_logo    = $vendorImage->getImage($value->v_id,$vendorImage->List_Logo);
				$store_icon    = $vendorImage->getImage($value->v_id,$vendorImage->Order_List_Logo);
		...
		return response()->json(['status' => 'my_order', 'data' => $data, 'logo_path' => env('ADMIN_URL'),'vendor_list' => $vendor, 'store_list' => $stores  ],200);
	}
}
