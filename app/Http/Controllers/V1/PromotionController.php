...
class PromotionController extends Controller
{
    ...
    public function process_individual_item($params){
        ...
        foreach ($push_data as $key => $value) {
            if ($key == 'BuyNofXforZ%offTiered') {
                $final_data = $this->buy_source_get_percentage_tiered($mrp, $qty, $value, $barcode, $store, $user_id, $cart_item);
            } elseif ($key == 'BuyNOrMoreOfXGetatUnitPriceTiered') {
                $final_data = $this->buy_at_per_kg($mrp, $qty, $value, $barcode, $store, $user_id, $cart_item);
            } elseif ($key == 'BuyNofXforZRs' || $key == 'BuyNofXforZ$') {
                $final_data = $this->buy_source_get_fixed_price($mrp, $qty, $value, $barcode, $store, $user_id, $cart_item);
            } elseif ($key == 'BuyNofXforZRsoff' || $key == 'BuyNofXforZ$off') {
                $final_data = $this->buy_source_get_amount($mrp, $qty, $value, $barcode, $store, $user_id, $cart_item);
            } elseif ($key == 'BuyNofXforZ%off') {
                $final_data = $this->buy_source_get_percentage($mrp, $qty, $value, $barcode, $store, $user_id, $cart_item);
            } elseif ($key == 'BuyNofXgetYatZRs' || $key == 'BuyNofXgetYatZ$') {

                $final_data = $this->buy_source_get_target_fixed_price($mrp, $qty, $value , $barcode, $store, $user_id, $cart_item);
            }elseif ($key == 'BuyNofXgetYatZ%off') {
                $final_data = $this->buy_source_get_target_percentage($mrp, $qty, $value , $barcode, $store, $user_id, $cart_item);
                
            }elseif ($key == 'BuyNofXgetYatZ$off') {
                $final_data = $this->buy_source_get_target_amount($mrp, $qty, $value , $barcode, $store, $user_id, $cart_item);
                
            }elseif ($key == 'BuyNofXforZ$Tiered' || $key == 'BuyNofXforZRsTiered') {
                //echo 'Inside this';exit;
                $final_data = $this->buy_source_get_fixed_price_tiered($mrp, $qty, $value , $barcode, $store, $user_id, $cart_item);
            }elseif ($key == 'BuyNofXgetLowestPricedXatZ%off') {
                //echo 'inside this';exit;
                $final_data = $this->buy_source_get_lowest_percentage_price($mrp, $qty, $value , $barcode, $store, $user_id, $cart_item);
            }elseif ($key == 'BuyNofXgetHighestPricedXatZ%off') {
                //echo 'inside this';exit;
                $final_data = $this->buy_source_get_higest_percentage_price($mrp, $qty, $value , $barcode, $store, $user_id, $cart_item);
            }elseif ($key == 'BuyNofXgetMofXwithLowestPriceatZ%off') {
                //echo 'inside this';exit;
                $final_data = $this->buy_source_get_source_lowest_percentage_price($mrp, $qty, $value , $barcode, $store, $user_id, $cart_item);
            }elseif ($key == 'Buy$NofXatZ$offTiered') {
                //echo 'inside this';exit;
                $final_data = $this->shop_amount_get_source_amount_tiered($mrp, $qty, $value , $barcode, $store, $user_id , $cart_item );
            }
        ...
        //dd($target_offer);
        $source_item = DB::table('spar_uat.item_master')->select('ITEM')->where('EAN', $barcode)->first();
       ...
       // dd($section_push_data);
        foreach ($section_push_data as $key => $value) {
            if ($key == 'BuyRsNOrMoreOfXGetYatZ%OffTiered') {
                ...
                foreach($value as $k => $val){
                            $senVal = [ $k => $val] ;
                    $section_final_data[] = $this->shop_amount_get_target_percentage_tiered($mrp, $qty, $senVal , $barcode, $store, $user_id , $cart_item );
                }
            }elseif ($key == 'Buy$NorMoreOfXgetYatZ$') {
                //echo 'inside this';exit;
                foreach($value as $k => $val){
                            $senVal = [ $k => $val] ;
                    $section_final_data[] = $this->shop_amount_get_target_fixed_price($mrp, $qty, $senVal , $barcode, $store, $user_id , $cart_item );
                }
            }elseif ($key == 'Buy$NorMoreOfXgetYatZ%off') {
                //echo 'inside this';exit;
                foreach($value as $k => $val){
                            $senVal = [ $k => $val] ;
                    $section_final_data[] = $this->shop_amount_get_target_percentage($mrp, $qty, $senVal , $barcode, $store, $user_id , $cart_item );
                }
            }elseif ($key == 'BuyRsNOrMoreOfXGetYatZRsOffTiered') {
                //echo 'inside this';exit;
                foreach($value as $k => $val){
                            $senVal = [ $k => $val] ;
                    $section_final_data[] = $this->shop_amount_get_target_amount_tiered($mrp, $qty, $senVal , $barcode, $store, $user_id , $cart_item );
                }
            }elseif ($key == 'BuyRsNOrMoreOfXGetYatZRsTiered') {
                //echo 'inside this';exit;
                foreach($value as $k => $val){
                            $senVal = [ $k => $val] ;
                    $section_final_data[] = $this->shop_amount_get_target_fixed_price_tiered($mrp, $qty, $senVal , $barcode, $store, $user_id , $cart_item );
                }
            }elseif ($key == 'Buy$NorMoreOfXgetYatZ$off') {
                //echo 'inside this';exit;
                foreach($value as $k => $val){
                            $senVal = [ $k => $val] ;
                    $section_final_data[] = $this->shop_amount_get_target_amount($mrp, $qty, $senVal , $barcode, $store, $user_id , $cart_item );
                }
            }
        }
        ...
        foreach ($final_data['pdata'] as $key => $value) {
            ...
            foreach ($tax_rates as $tkey => $tax_rate) {
                ...
                $taxable_amount = round ( $value['ex_price'] * $tax_rate->TXBL_FCT , 2 );
                $tax = round( ( $taxable_amount / 100  ) * $tax_rate->TX_RT , 2 );
                $taxes[] = [ 'tax_desc' => $tax_rate->TX_CD_DSCR , 'tax_code' => $tax_rate->TX_CD , 'tax_rate' => $tax_rate->TX_RT , 'taxable_factor' => $tax_rate->TXBL_FCT , 'taxable_amount' => $taxable_amount , 'tax' => $tax ];
                $total_tax += $tax;
            }
    ...
    public function buy_at_per_kg($mrp, $qty, $ru_prdv, $item_id, $store_id, $user_id, $cart_item= false)
    {
        // dd($ru_prdv);
        $source_item = DB::table('spar_uat.item_master')->select('ITEM')->where('EAN', $item_id)->first();
        $data = [];
        $final_data = [];
        $final_product = [];
        foreach ($ru_prdv as $key => $val) {
            foreach ($val as $key => $value) {
                if ($value['qu_th'] != 0.00) {
                    $condition = DB::table('spar_uat.co_prdv_itm')->select('PNT_PRC_UN_ITM_PRDV_SLS')->where('ID_RU_PRDV', $value['ru_prdv'])->first();
        ...
        if ($data[$max_discount]['TYPE'] == 'max') {
            $list_ru_prdv = DB::table('spar_uat.ru_prdv')
                            ->select('spar_uat.ru_prdv.ID_RU_PRDV', 'spar_uat.max_co_el_prdv_itm_grp.MO_TH', 'spar_uat.max_co_el_prdv_itm_grp.QU_TH', 'spar_uat.co_prdv_itm.PNT_PRC_UN_ITM_PRDV_SLS', 'spar_uat.max_co_el_prdv_itm_grp.ID_GRP','spar_uat.ru_prdv.ID_PRM')
                            ->join('spar_uat.max_co_el_prdv_itm_grp', 'spar_uat.ru_prdv.ID_RU_PRDV', '=', 'spar_uat.max_co_el_prdv_itm_grp.ID_RU_PRDV')
                            ->join('spar_uat.co_prdv_itm', 'spar_uat.ru_prdv.ID_RU_PRDV', '=', 'spar_uat.co_prdv_itm.ID_RU_PRDV')
                            ->where('spar_uat.ru_prdv.ID_PRM', $data[$max_discount]['ID_PRM'])
                            ->get();
    ...
    public function buy_source_get_fixed_price($mrp, $qty, $ru_prdv, $item_id, $store_id, $user_id, $cart_item= false)
    {
        // dd($ru_prdv);
        $source_item = DB::table('spar_uat.item_master')->select('ITEM')->where('EAN', $item_id)->first();
        if($cart_item){
          ...
          $check_product_in_cart = DB::table('cart')->where('barcode', $item_id)->where('user_id', $user_id)->where('store_id', $store_id)->first();
        }
        ...
        if ($data[$max_discount]['TYPE'] == 'max') {
            $list_ru_prdv = DB::table('spar_uat.ru_prdv')
                            ->select('spar_uat.ru_prdv.ID_RU_PRDV', 'spar_uat.max_co_el_prdv_itm_grp.MO_TH', 'spar_uat.max_co_el_prdv_itm_grp.QU_TH', 'spar_uat.ru_prdv.CD_MTH_PRDV', 'spar_uat.co_prdv_itm.PNT_PRC_UN_ITM_PRDV_SLS')
                            ->join('spar_uat.max_co_el_prdv_itm_grp', 'spar_uat.ru_prdv.ID_RU_PRDV', '=', 'spar_uat.max_co_el_prdv_itm_grp.ID_RU_PRDV')
                            ->join('spar_uat.co_prdv_itm', 'spar_uat.ru_prdv.ID_RU_PRDV', '=', 'spar_uat.co_prdv_itm.ID_RU_PRDV')
                            ->where('spar_uat.ru_prdv.ID_PRM', $data[$max_discount]['ID_PRM'])
                            ->get();
    ...
    public function buy_source_get_amount($mrp, $qty, $ru_prdv, $item_id, $store_id, $user_id, $cart_item= false)
    {
        // dd($ru_prdv);
        $source_item = DB::table('spar_uat.item_master')->select('ITEM')->where('EAN', $item_id)->first();
        if($cart_item){
        ...
        foreach ($ru_prdv as $key => $val) {
            foreach ($val as $key => $value) {
                $condition = DB::table('spar_uat.co_prdv_itm')->select('MO_UN_ITM_PRDV_SLS')->where('ID_RU_PRDV', $value['ru_prdv'])->first();
        ...
        if ($data[$max_discount]['TYPE'] == 'max') {
            $list_ru_prdv = DB::table('spar_uat.ru_prdv')
                            ->select('spar_uat.ru_prdv.ID_RU_PRDV', 'spar_uat.max_co_el_prdv_itm_grp.MO_TH', 'spar_uat.max_co_el_prdv_itm_grp.QU_TH', 'spar_uat.ru_prdv.CD_MTH_PRDV', 'spar_uat.co_prdv_itm.MO_UN_ITM_PRDV_SLS')
                            ->join('spar_uat.max_co_el_prdv_itm_grp', 'spar_uat.ru_prdv.ID_RU_PRDV', '=', 'spar_uat.max_co_el_prdv_itm_grp.ID_RU_PRDV')
                            ->join('spar_uat.co_prdv_itm', 'spar_uat.ru_prdv.ID_RU_PRDV', '=', 'spar_uat.co_prdv_itm.ID_RU_PRDV')
                            ->where('spar_uat.ru_prdv.ID_PRM', $data[$max_discount]['ID_PRM'])
                            ->get();
    ...
    public function buy_source_get_percentage($mrp, $qty, $ru_prdv, $item_id, $store_id, $user_id, $cart_item= false)
    {
        // dd($ru_prdv);
        $source_item = DB::table('spar_uat.item_master')->select('ITEM')->where('EAN', $item_id)->first();
        if($cart_item){
        ...
        foreach ($ru_prdv as $key => $val) {
            foreach ($val as $key => $value) {
                $condition = DB::table('spar_uat.co_prdv_itm')->select('PE_UN_ITM_PRDV_SLS')->where('ID_RU_PRDV', $value['ru_prdv'])->first();
        ...
        if ($data[$max_discount]['TYPE'] == 'max') {
            $list_ru_prdv = DB::table('spar_uat.ru_prdv')
                            ->select('spar_uat.ru_prdv.ID_RU_PRDV', 'spar_uat.max_co_el_prdv_itm_grp.MO_TH', 'spar_uat.max_co_el_prdv_itm_grp.QU_TH', 'spar_uat.ru_prdv.CD_MTH_PRDV', 'spar_uat.co_prdv_itm.PE_UN_ITM_PRDV_SLS')
                            ->join('spar_uat.max_co_el_prdv_itm_grp', 'spar_uat.ru_prdv.ID_RU_PRDV', '=', 'spar_uat.max_co_el_prdv_itm_grp.ID_RU_PRDV')
                            ->join('spar_uat.co_prdv_itm', 'spar_uat.ru_prdv.ID_RU_PRDV', '=', 'spar_uat.co_prdv_itm.ID_RU_PRDV')
                            ->where('spar_uat.ru_prdv.ID_PRM', $data[$max_discount]['ID_PRM'])
                            ->get();
    ...
    public function buy_source_get_target_fixed_price($mrp, $qty, $ru_prdv, $item_id, $store_id = '20001', $user_id='test', $cart_item= false)
    {
        //dd($ru_prdv);
        $source_item = DB::table('spar_uat.item_master')->select('ITEM')->where('EAN', $item_id)->first();
        ...
        foreach ($ru_prdv as $key => $val) {
            foreach ($val as $key => $value) {
                $condition = DB::table('spar_uat.tr_itm_mxmh_prdv')->select('PNT_PRC_RDN_MXMH as PNT_PRC_UN_ITM_PRDV_SLS', 'ID_PRM_PRD','QU_LM_MXMH')->where('ID_RU_PRDV', $value['ru_prdv'])->first();
                if($condition){

                    if($value['cd_bas_cmp_tgt'] == 7){
                        $grp_list = DB::table('spar_uat.max_grp_itm_lst')->select('ID_ITM')->where('ID_GRP', $condition->ID_PRM_PRD )->first();
                        $target_product = DB::table('spar_uat.price_master')->where('ITEM', $grp_list->ID_ITM)->first();
                       
                    }else{

                        $target_product = DB::table('spar_uat.price_master')->where('ITEM', $condition->ID_PRM_PRD)->first();
                    }
        ...
        if ($data[$max_discount]['TYPE'] == 'max' ){
            $list_ru_prdv = DB::table('spar_uat.ru_prdv')
                            ->select('spar_uat.ru_prdv.ID_RU_PRDV','spar_uat.ru_prdv.QU_AN_SRC', 'spar_uat.ru_prdv.QU_AN_TGT','spar_uat.ru_prdv.MAX_ALL_SOURCES','spar_uat.ru_prdv.ITM_PRC_CTGY_SRC', 'spar_uat.max_co_el_prdv_itm_grp.MO_TH', 'spar_uat.max_co_el_prdv_itm_grp.QU_TH', 'spar_uat.tr_itm_mxmh_prdv.QU_LM_MXMH', 'spar_uat.ru_prdv.CD_MTH_PRDV','spar_uat.max_grp_itm_lst.ID_ITM','spar_uat.tr_itm_mxmh_prdv.PNT_PRC_RDN_MXMH as PNT_PRC_UN_ITM_PRDV_SLS')
                            ->join('spar_uat.max_co_el_prdv_itm_grp', 'spar_uat.ru_prdv.ID_RU_PRDV', '=', 'spar_uat.max_co_el_prdv_itm_grp.ID_RU_PRDV')
                            ->join('spar_uat.max_grp_itm_lst', 'max_co_el_prdv_itm_grp.ID_GRP', '=', 'max_grp_itm_lst.ID_GRP')
                            ->join('spar_uat.tr_itm_mxmh_prdv', 'ru_prdv.ID_RU_PRDV', '=', 'tr_itm_mxmh_prdv.ID_RU_PRDV')
                            ->where('spar_uat.ru_prdv.ID_PRM', $data[$max_discount]['ID_PRM'])
        ...
        if ( $data[$max_discount]['TYPE'] == 'co_el') {
            $list_ru_prdv = DB::table('spar_uat.ru_prdv')
                            ...
                            ->join('spar_uat.co_el_prdv_itm', 'ru_prdv.ID_RU_PRDV', '=', 'co_el_prdv_itm.ID_RU_PRDV')
                            ->join('spar_uat.tr_itm_mxmh_prdv', 'ru_prdv.ID_RU_PRDV', '=', 'tr_itm_mxmh_prdv.ID_RU_PRDV')
    ...
    public function buy_source_get_target_percentage($mrp, $qty, $ru_prdv, $item_id, $store_id = '20001', $user_id='test', $cart_item= false)
    {
        //dd($ru_prdv);
        $source_item = DB::table('spar_uat.item_master')->select('ITEM')->where('EAN', $item_id)->first();
        $carts = DB::table('cart')->where('user_id', $user_id)->where('store_id', $store_id)->get();
        ...
        foreach ($ru_prdv as $key => $val) {
            foreach ($val as $key => $value) {
                $condition = DB::table('spar_uat.tr_itm_mxmh_prdv')->select('PE_RDN_PRC_MXMH as PE_UN_ITM_PRDV_SLS', 'ID_PRM_PRD','QU_LM_MXMH')->where('ID_RU_PRDV', $value['ru_prdv'])->first();
        ...
        if ($data[$max_discount]['TYPE'] == 'max') {
            $list_ru_prdv = DB::table('spar_uat.ru_prdv')
                            ...
                            ->join('spar_uat.max_co_el_prdv_itm_grp', 'ru_prdv.ID_RU_PRDV', '=', 'max_co_el_prdv_itm_grp.ID_RU_PRDV')
                            ->join('spar_uat.max_grp_itm_lst', 'max_co_el_prdv_itm_grp.ID_GRP', '=', 'max_grp_itm_lst.ID_GRP')
                            ->join('spar_uat.tr_itm_mxmh_prdv', 'ru_prdv.ID_RU_PRDV', '=', 'tr_itm_mxmh_prdv.ID_RU_PRDV')
                            ->where('ru_prdv.ID_PRM', $data[$max_discount]['ID_PRM'])
                            //->where('co_el_prdv_itm.ID_ITM', $item_id)
    ...
    public function buy_source_get_target_amount($mrp, $qty, $ru_prdv, $item_id, $store_id = '20001', $user_id='test', $cart_item= false)
    {
        //dd($ru_prdv);
        $source_item = DB::table('spar_uat.item_master')->select('ITEM')->where('EAN', $item_id)->first();
        $carts = DB::table('cart')->where('user_id', $user_id)->where('store_id', $store_id)->get();
        ...
        foreach ($ru_prdv as $key => $val) {
            foreach ($val as $key => $value) {
                $condition = DB::table('spar_uat.tr_itm_mxmh_prdv')->select('MO_RDN_PRC_MXMH as MO_UN_ITM_PRDV_SLS', 'ID_PRM_PRD','QU_LM_MXMH')->where('ID_RU_PRDV', $value['ru_prdv'])->first();
    ...
    public function buy_source_get_percentage_tiered($mrp, $qty, $ru_prdv, $item_id, $store_id = '20001', $user_id='test', $cart_item= false)
    {
        //dd($ru_prdv);
        $source_item = DB::table('spar_uat.item_master')->select('ITEM')->where('EAN', $item_id)->first();
        $carts = DB::table('cart')->where('user_id', $user_id)->where('store_id', $store_id)->get();
        ...
        if ($data[$max_discount]['TYPE'] == 'max') {
            $list_ru_prdv = DB::table('spar_uat.ru_prdv')
                            ...
                            ->join('spar_uat.max_co_el_prdv_itm_grp', 'ru_prdv.ID_RU_PRDV', '=', 'max_co_el_prdv_itm_grp.ID_RU_PRDV')
                            ->join('spar_uat.max_grp_itm_lst', 'max_co_el_prdv_itm_grp.ID_GRP', '=', 'max_grp_itm_lst.ID_GRP')
                            ->join('spar_uat.co_prdv_itm', 'ru_prdv.ID_RU_PRDV', '=', 'co_prdv_itm.ID_RU_PRDV')
                            ->where('ru_prdv.ID_PRM', $data[$max_discount]['ID_PRM'])
                            //->where('co_el_prdv_itm.ID_ITM', $item_id)
                            ->get();
        }
    ...
    public function buy_source_get_fixed_price_tiered($mrp, $qty, $ru_prdv, $item_id, $store_id = '20001', $user_id='test',$cart_item= false)
    {
        //dd($ru_prdv);
        $source_item = DB::table('spar_uat.item_master')->select('ITEM')->where('EAN', $item_id)->first();
        $carts = DB::table('cart')->where('user_id', $user_id)->where('store_id', $store_id)->get();
                    ...
                    if(!empty($combination)){
                        ...
                        foreach($combination as $key => $combi){
                            ...
                            foreach($combi as $combi_qty){
                                ...
                                $id = $this->searchQty($combi_qty / $add_qty, $list_ru_prdv);
                                $total_price += $mrp * $combi_qty;
                                $ex_price += $id->PNT_PRC_UN_ITM_PRDV_SLS;
                                $discount += $total_price - $ex_price; 
                                array_push($best_final_product,[ 'qty' => $combi_qty, 'is_promo' => 1 ]);
                            }
                            if($temp_qty > 0 ){
            ...
            foreach ($final_product as $key => $value) {
                if ($value['is_promo'] == 1) {
                    ...
                    while($loopQty > 0){
                        ...
                        //$discount = $total_price - $ex_price;  
                        $discount = round( ($ratio_val[$param_count_of_cart]/$ratio_total) * $offer_amount ,2);
                        $ex_price = $total_price - $discount;   
                        $loopQty--;
                        $param_count_of_cart++;         
    ...
    public function buy_source_get_lowest_percentage_price($mrp, $qty, $ru_prdv, $item_id, $store_id = '20001', $user_id='test',$cart_item= false){
        ...
        if ($data[$max_discount]['TYPE'] == 'max') {
            ...
            foreach ($final_product as $key => $value) {
                if ($value['is_promo'] == 1) {
                    ...
                    if($cart_source_qty > 0){
                        //$value['qty'] -= $cart_source_qty;
                        $cart_source_item = $cart_source_item->sortByDesc('unit_mrp');
                        //dd($cart_source_item);
                        
                        $cart_source_item = $cart_source_item->filter(function($q , $key) use (&$value, &$mrp_on_offer, &$offer_amount, &$param) {
    ...
    public function buy_source_get_source_lowest_percentage_price($mrp, $qty, $ru_prdv, $item_id, $store_id = '20001', $user_id='test', $cart_item= false){
        ...
        if ($data[$max_discount]['TYPE'] == 'co_el') {
            $list_ru_prdv = DB::table('spar_uat.ru_prdv')
                            ->select('ru_prdv.ID_RU_PRDV','ru_prdv.CD_BAS_CMP_SRC','ru_prdv.QU_AN_SRC','ru_prdv.QU_AN_TGT','ru_prdv.MAX_ALL_SOURCES','ru_prdv.ITM_PRC_CTGY_SRC', 'ru_prdv.RULE_QTY', 'co_el_prdv_itm.MO_TH', 'co_el_prdv_itm.QU_TH','co_el_prdv_itm.ID_ITM', 'ru_prdv.CD_MTH_PRDV', 'co_prdv_itm.PE_UN_ITM_PRDV_SLS')
                            ->join('spar_uat.co_el_prdv_itm', 'ru_prdv.ID_RU_PRDV', '=', 'co_el_prdv_itm.ID_RU_PRDV')
                            ->join('spar_uat.co_prdv_itm', 'ru_prdv.ID_RU_PRDV', '=', 'co_prdv_itm.ID_RU_PRDV')
                            ->where('ru_prdv.ID_PRM', $data[$max_discount]['ID_PRM'])
                            //->where('co_el_prdv_itm.ID_ITM', $item_id)
        ...
        if ($data[$max_discount]['TYPE'] == 'max') {
            $list_ru_prdv = DB::table('spar_uat.ru_prdv')
                            ->select('ru_prdv.ID_RU_PRDV','ru_prdv.CD_BAS_CMP_SRC','ru_prdv.QU_AN_SRC', 'ru_prdv.QU_AN_TGT','ru_prdv.MAX_ALL_SOURCES','ru_prdv.ITM_PRC_CTGY_SRC','ru_prdv.RULE_QTY', 'max_co_el_prdv_itm_grp.MO_TH', 'max_co_el_prdv_itm_grp.QU_TH', 'co_prdv_itm.PE_UN_ITM_PRDV_SLS', 'ru_prdv.CD_MTH_PRDV','max_grp_itm_lst.ID_ITM')
                            ->join('spar_uat.max_co_el_prdv_itm_grp', 'ru_prdv.ID_RU_PRDV', '=', 'max_co_el_prdv_itm_grp.ID_RU_PRDV')
                            ->join('spar_uat.max_grp_itm_lst', 'max_co_el_prdv_itm_grp.ID_GRP', '=', 'max_grp_itm_lst.ID_GRP')
                            ->join('spar_uat.co_prdv_itm', 'ru_prdv.ID_RU_PRDV', '=', 'co_prdv_itm.ID_RU_PRDV')
                            ->where('ru_prdv.ID_PRM', $data[$max_discount]['ID_PRM'])
            ...
            foreach ($final_product as $key => $value) {
                if ($value['is_promo'] == 1) {
                    ...
                    $ratio_val = $this->get_offer_amount_by_ratio($param, $offer_amount);
                    $ratio_total = array_sum($ratio_val);
                    //$final_discount = ($var[0]/$ratio_total) * $discount;
    ...
    public function buy_source_get_higest_percentage_price($mrp, $qty, $ru_prdv, $item_id, $store_id = '20001', $user_id='test', $cart_item= false){
    ...
    public function shop_amount_get_source_amount_tiered($mrp, $qty, $ru_prdv, $item_id, $store_id = '20001', $user_id='test',$cart_item = true)
    {
        //dd($ru_prdv);
        $source_item = DB::table('spar_uat.item_master')->select('ITEM')->where('EAN', $item_id)->first();
        $carts = DB::table('cart')->where('user_id', $user_id)->where('store_id', $store_id)->get();
        ...
        if ($data[$max_discount]['TYPE'] == 'max') {
            ...
            foreach ($amount_collection as $amount) {
                if($cart_totals >= $amount ){
                    $value = [ 'amount' => $amount ,  'is_promo' => 1];
                }
            }
            ...
            //foreach ($final_product as $key => $value) {
                if ($value['is_promo'] == 1) {
                    ...
                    /*if($cart_source_qty > 0){
                        ...
                        $offer_amount = $this->get_offer_amount_by_ratio($param, $offer_amount);
                       //exit;
                    }*/
                    ...
                    $final_data['pdata'][] = [ 'qty' => $qty, 'mrp' => $mrp, 'discount' => $discount, 'ex_price' => $ex_price, 'total_price' => $total_price, 'message' => $message , 'ru_prdv' => $id->ID_RU_PRDV, 'type' => $data[$max_discount]['TYPE'], 'promo_id' => $data[$max_discount]['ID_PRM'], 'type_id' => $data[$max_discount]['ID'], 'is_slab' => ($promo_count == 1 ? 0 : 2), 'is_promo' => 1 ];
        ...
        if($all_sources_check){
            ...
            foreach ($final_product as $key => $value) {
                if ($value['is_promo'] == 1) {
                    ...
                    $message = 'buy '.$value['qty'].' each , get '.$id->QU_LM_MXMH.' '.$price_master->ITEM_DESC.' for '.$id->PE_UN_ITM_PRDV_SLS.' % Off';
                    $final_data['pdata'][] = [ 'qty' => $value['qty'], 'mrp' => $mrp, 'discount' => $discount, 'ex_price' => $ex_price, 'total_price' => $total_price, 'message' => $message , 'ru_prdv' => '', 'type' => '', 'promo_id' => '', 'type_id' => '', 'is_slab' => 0, 'is_promo' => 1 ];
                        $applied_offer[] = $message;
    ...
    public function shop_amount_get_target_percentage_tiered($mrp, $qty, $ru_prdv, $item_id, $store_id = '20001', $user_id='test', $cart_item = false)
    {
        //dd($ru_prdv);
        $source_item = DB::table('spar_uat.item_master')->select('ITEM')->where('EAN', $item_id)->first();
        $carts = DB::table('cart')->where('user_id', $user_id)->where('store_id', $store_id)->get();
    ...
    public function shop_amount_get_target_fixed_price($mrp, $qty, $ru_prdv, $item_id, $store_id = '20001', $user_id='test', $cart_item = false)
    {
        //dd($ru_prdv);
        $source_item = DB::table('spar_uat.item_master')->select('ITEM')->where('EAN', $item_id)->first();
        $carts = DB::table('cart')->where('user_id', $user_id)->where('store_id', $store_id)->get();
    ...
    public function shop_amount_get_target_percentage($mrp, $qty, $ru_prdv, $item_id, $store_id = '20001', $user_id='test', $cart_item = false)
    {
        //dd($ru_prdv);
        $source_item = DB::table('spar_uat.item_master')->select('ITEM')->where('EAN', $item_id)->first();
        $carts = DB::table('cart')->where('user_id', $user_id)->where('store_id', $store_id)->get();
    ...
    public function shop_amount_get_target_amount_tiered($mrp, $qty, $ru_prdv, $item_id, $store_id = '20001', $user_id='test', $cart_item = false)
    {
        //dd($ru_prdv);
        $source_item = DB::table('spar_uat.item_master')->select('ITEM')->where('EAN', $item_id)->first();
        $carts = DB::table('cart')->where('user_id', $user_id)->where('store_id', $store_id)->get();
    ...
    public function shop_amount_get_target_fixed_price_tiered($mrp, $qty, $ru_prdv, $item_id, $store_id = '20001', $user_id='test', $cart_item = false)
    {
        //dd($ru_prdv);
        $source_item = DB::table('spar_uat.item_master')->select('ITEM')->where('EAN', $item_id)->first();
        $carts = DB::table('cart')->where('user_id', $user_id)->where('store_id', $store_id)->get();
    ...
    public function calculate_shop_target_offer_of_fixed_price_tiered($param){
     ...
     public function shop_amount_get_target_amount($mrp, $qty, $ru_prdv, $item_id, $store_id = '20001', $user_id='test', $cart_item = false)
    {
        //dd($ru_prdv);
        $source_item = DB::table('spar_uat.item_master')->select('ITEM')->where('EAN', $item_id)->first();
        $carts = DB::table('cart')->where('user_id', $user_id)->where('store_id', $store_id)->get();
    ...
    public function shop_bill_get_printed_tiered($total_amount, $qty, $ru_prdv, $item_id, $store_id = '20001', $user_id='test',$cart_item = true){
        ...
        foreach ($ru_prdv as $key => $val) {
            foreach ($val as $key => $value) {
                ...
                $matchProduct = DB::table('spar_uat.price_master')->select('ITEM','ITEM_DESC')->where('ITEM', $value['max_free_item'] )->first();
                dd($matchProduct);
