...
class CartController extends Controller
{
    ...
    public function cart_details(Request $request)
    {
            ...
            foreach ($carts as $key => $cart) {
                ...
                $product_data['p_name'] = str_replace('\n',' ',$cart->item_name);
                ...
                $images = DB::table($store_db_name.'.price_master')->where('ITEM', $cart->item_id)->select('IMAGE')->first();
                if(isset($images->IMAGE)){
                        
                    //$imagePath = product_image_link().$v_id.'/'.$images->IMAGE;
                    if(empty($images->IMAGE) || $images->IMAGE =='' || $images->IMAGE == null){
                        //echo 'inside this '.$images->IMAGE;exit;
                    }else{
                        $product_image = $v_id.'/'.$images->IMAGE;
                    }
            ...
            foreach ($vouchers as $key => $voucher) {
                array_push($voucher_array ,['name' => 'Star Credit' , 'amount' => $voucher->amount ] );
    ...
    public function process_to_payment(Request $request)
    {
        ...
        if($total == 0.00){
            ...
            return response()->json(['status' => 'payment_save', 'go_to' => 'item_receipt' , 'message' => 'Save Payment', 'data' => $payment , 'redirect_to_qr' => true ],200);
            exit;
    ...
    public function payment_details(Request $request)
    {
        ...
        if($payment_save_status){
            ...
            return response()->json(['status' => 'payment_save', 'go_to' => 'item_receipt' , 'message' => 'Save Payment', 'data' => $payment ],200);
        }
    }
    ...
    public function order_details(Request $request)
    {
        ...
        return response()->json(['status' => 'order_details', 'message' => 'Order Details Details', 
            ...
            'store_header_logo' => store_logo_link().'starbazaar_logo_round.png',
            //'offer_data' => $global_offer_data,
    ...
    public function get_print_receipt(Request $request){
        ...
        foreach ($carts as $key => $cart) {
            ...
            $tax_details_item =[ 'taxable_amount' => 0 , 'cgst' => 0, 'sgst' => 0 , 'cgst_rate' => '0' , 'sgst_rate' => '0' ];
            ...
            foreach ($offer_data['pdata'] as $key => $value) {
                ...
                if(empty($value['tax']) ){
                    ...
                    $tax_details_item['taxable_amount'] += $value['tax'][0]['taxable_amount'] ;

                    foreach($value['tax'] as $nkey => $tax){

                        
                        if(strpos($tax['tax_desc'], 'CGST') !== false ){
                            $tax_details_item['cgst'] += $tax['tax'];
                            $tax_details_item['cgst_rate'] = $tax['tax_rate'];
                        }

                        if(strpos($tax['tax_desc'], 'SGST') !== false ){
                             $tax_details_item['sgst'] += $tax['tax'];
                             $tax_details_item['sgst_rate'] = $tax['tax_rate'];
                        }
            ...
            $cart_item_arr[] = ['row' => '1' , 'tax_details' => 'CGST @ '.$tax_details_item['cgst_rate'].'% SGST @'.$tax_details_item['sgst_rate'].'%' ];
            $cart_item_arr[] = ['row' => '2' , 'cart_item_name' => substr(str_replace('\n',' ',$cart->item_name),0, 20 )  ];
            $cart_item_arr[] = ['row' => '3' , 'hsn_code' => $hsn_code , 'qty' =>$cart->qty , 'unit_mrp' => format_number($cart->unit_mrp) , 'total' => $cart->total ];
            ...
            $tax_details_item['total'] = $tax_details_item['cgst'] + $tax_details_item['sgst'];
            $tax_details_indv[] = $tax_details_item;
        ...
        $cashier = DB::table('vender_users_auth')->where('vu_id', $order->vu_id)->first();
        ...
        $print_data_array['store_name'] = 'Star HYPERMARKET INDIA PVT LTD';
        $print_data_array['header1'] = ['title' => $stores->location , 'data' => 'CIN - '.$stores->tin.'\n'.'GSTIN - '.$stores->gst ] ;
        $print_data_array['header2'] = $stores->address1.' \n'.$stores->address2.' \n'.$stores->city.' - '.$stores->pincode;
        $cashier_name = (isset($cashier->first_name))?$cashier->first_name:'';
        $tax_invoice_data = 'BILL.NO:'.$order->order_id.'\nCASHIER :'. $cashier_name.'\n'.'BILL.DATE:'.$order->date.' TIME:'.$order->time;
        

        $print_data_array['header3'] = [
            'title' => ($rounded>=200)?'TAX INVOICE':'BILL OF SUPPLY' , 
            'data' =>  $tax_invoice_data] ;
        ...
        $print_data_array['cart_item_summary'] = 'ITEM :'.$counter.' QTY : '.$total_qty.' AMT : '.format_number($rounded);

        $print_data_array['gst_breakup_details'] = $tax_details_indv;

        $gst_breakup_total = ['taxable_amount' => 0,  'cgst' => 0 , 'sgst' => 0, 'total' => 0 ];
        foreach($tax_details_indv as $breakup){
            $gst_breakup_total['taxable_amount'] += $breakup['taxable_amount'];
            $gst_breakup_total['cgst'] += $breakup['cgst'];
            $gst_breakup_total['sgst'] += $breakup['sgst'];
            $gst_breakup_total['total'] += $breakup['total'];
        }

        $gst_breakup_total = [ 'taxable_amount' => number_format((float)$gst_breakup_total['taxable_amount']),
          'cgst' => number_format((float)$gst_breakup_total['cgst']),
          'sgst' => number_format((float)$gst_breakup_total['sgst']),
          'total' => number_format((float)$gst_breakup_total['total'])
           ];
        $print_data_array['gst_breakup_total'] = $gst_breakup_total;
        $payment = DB::table('payments')->where('order_id', $order->order_id)->first();

        $print_data_array['footer1'] = '--AMOUNT RECEIVED FROM CUSTOMER-\n     '.$payment->method.'    '.format_number($rounded).'\nABOVE PRICES ARE INCLUSIVE OF ALL TAXES. \nTHIS IS COMPUTER GENERATED INVOCE AND HENCE NO SIGNATURE REQUIRED';
        $print_data_array['footer2'] = 'FOR FEEBACK / COMPLAINTS PLEASE WRITE US AT \nsuggesstion@Starindia.com';
        /*$print_data_array['gst_breakup_total'] = $gst_breakup_total;
        ...
        <html>
            ...
            <body>
                ...
                <center>
                    ...
                    <p>Star HYPERMARKET INDIA PVT LTD</P>
                    <hr/>
                    <div class="invoice-address">
                        ...
                        <p>EMAIL - customer@Starindia.com</P>
