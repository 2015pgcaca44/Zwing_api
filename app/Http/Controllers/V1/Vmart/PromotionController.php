...
class PromotionController extends Controller
{
    public function final_check_promo_sitewise($request, $cart = 0)
    {
        ...
        $count = 0;
        $item = DB::table('vmart.invitem')->select('GRPCODE','INVARTICLE_CODE','CCODE1','CCODE2','CCODE3','CCODE4','CCODE5','CCODE6','ICODE','GENERATED','MRP','CNAME1','CNAME2','INVHSNSACMAIN_CODE')->where('ICODE', $id)->first();
        ...
        if(count($psite_promo_assign) > 0) {
            foreach ($psite_promo_assign as $key => $promo_assign) {
                ...
                // if (($today >= $startdate) && ($today <= $enddate)) {
                    // echo 'Start Date :- '.$startdate.' End Date :- '.$enddate.' CODE :- '.$promo_assign->PROMO_CODE.'<br>';
                    $promo_buy = DB::table('vmart.promo_buy')->select('ASSORTMENT_CODE')->where('PROMO_CODE', $promo_assign->PROMO_CODE)->first();
        ...
        // dd($promo_data);
        $adivision = DB::table('vmart.promo_assortment_include')
        ...
        $asection = DB::table('vmart.promo_assortment_include')
                    ->select('ASSORTMENT_CODE')
        ...
        $adepartment = DB::table('vmart.promo_assortment_include')
                    ->select('ASSORTMENT_CODE')
        ...
        $aarticle = DB::table('vmart.promo_assortment_include')
                    ->select('ASSORTMENT_CODE')
        ...
        $item_code = DB::table('vmart.promo_assortment_include')
                    ->select('ASSORTMENT_CODE')
                    ->where('ICODE', $id)
                    ->whereIn('ASSORTMENT_CODE', $promo_data)
                    ->orderBy('ASSORTMENT_CODE','DESC')
        ...
        if (!empty($item->CCODE1)) {
            $category_1 = DB::table('vmart.promo_assortment_include')
                        ->select('ASSORTMENT_CODE')
        ...
        if (!empty($item->CCODE2)) {
            $category_2 = DB::table('vmart.promo_assortment_include')
                        ->select('ASSORTMENT_CODE')
                        ->where('CCODE1', $item->CCODE1)
                        ->whereIn('ASSORTMENT_CODE', $promo_data)
                        ->orderBy('ASSORTMENT_CODE','DESC')
        ...
        if (!empty($item->CCODE3)) {
            $category_3 = DB::table('vmart.promo_assortment_include')
                        ->select('ASSORTMENT_CODE')
                        ->where('CCODE1', $item->CCODE1)
                        ->whereIn('ASSORTMENT_CODE', $promo_data)
                        ->orderBy('ASSORTMENT_CODE','DESC')
        ...
        if (!empty($item->CCODE4)) {
            $category_4 = DB::table('vmart.promo_assortment_include')
                        ->select('ASSORTMENT_CODE')
                        ->where('CCODE1', $item->CCODE1)
                        ->whereIn('ASSORTMENT_CODE', $promo_data)
                        ->orderBy('ASSORTMENT_CODE','DESC')
        ...
        if (!empty($item->CCODE5)) {
            $category_5 = DB::table('vmart.promo_assortment_include')
                        ->select('ASSORTMENT_CODE')
                        ->where('CCODE1', $item->CCODE1)
                        ->whereIn('ASSORTMENT_CODE', $promo_data)
                        ->orderBy('ASSORTMENT_CODE','DESC')
        ...
        if (!empty($item->CCODE6)) {
            $category_6 = DB::table('vmart.promo_assortment_include')
                        ->select('ASSORTMENT_CODE')
                        ->where('CCODE1', $item->CCODE1)
                        ->whereIn('ASSORTMENT_CODE', $promo_data)
                        ->orderBy('ASSORTMENT_CODE','DESC')
        ...
        foreach ($unique_data as $key => $value) {
            ...
            $promobuy = DB::table('vmart.promo_buy')->select('ASSORTMENT_NAME','PROMO_CODE')->where('ASSORTMENT_CODE', $split_value[0])->whereIn('PROMO_CODE', $psite_promo_list)->first();
            ...
            case 'F':
                switch ($value->GET_BENEFIT_CODE) {
                    ...
                    case '2':
                        ...
                        if ($qty == 1) {
                        	// dd($sort_code);
                        	$promo_assortment_include = DB::table('vmart.promo_assortment_include')->where('ASSORTMENT_CODE', $sort_code[0])->where('CODE', $sort_code[1])->first();
                        	...
                        	if (!$check_cart->isEmpty()) {
                        		foreach ($check_cart as $key => $cart_value) {
                        			// $get_product = DB::table('vmart.invitem')->where('INVARTICLE_CODE', $promo_assortment_include->INVARTICLE_CODE)->where('ICODE', $id)->first();
                        			// dd($get_product);
                        			if ($cart_value->discount == '0.00' && $cart_value->item_id == $id) {
                                        ...
                                        $getall_barcode = DB::table('vmart.promo_assortment_include')->select('ICODE')->where('ASSORTMENT_CODE', $sort_code[0])->whereNotNull('ICODE')->get();
                                        $allbarcode = [];
                                        // dd($getall_barcode);
                                        foreach ($getall_barcode as $key => $getall_barcode_value) {
                                            $allbarcode[] = $getall_barcode_value->ICODE;
                                        }
                                        $get_product_from_article = DB::table('vmart.invitem')->select('ICODE')->where('GRPCODE', $promo_assortment_include->DEPARTMENT_GRPCODE)->get();
