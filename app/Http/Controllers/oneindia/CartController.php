...
//  Using Ginesys's Cart controller as parent controller
	...
	public function get_print_receipt(Request $request) 
	{
		...
		$printInvioce->addLine('(BRAND OWNED BY NYSAA RETAIL PVTLTD)', 22,true);
		$printInvioce->addLine('NYSAA RETAIL PVTLTD ('.$store->location.')', 22);

        $printInvioce->addLine($site_details->ADDRESS.','.$site_details->CTNAME.','.$site_details->PIN, 22);
        $printInvioce->addLine('Customer Care No.: '.$store->contact_number, 22);
        ...
        // Closes Left & Start center
        $printInvioce->addDivider('-', 20);
        $printInvioce->tableStructure(['Sl', 'Product','Qty','Uom','Rate','Disc.', 'Amt'], [3, 7,4,4,4,6,6], 22);
        $printInvioce->tableStructure(['Barcode', 'Hsn Code', 'Sales Man'], [16, 8, 10], 22);
        $printInvioce->addDivider('-', 20);
		...
		$printInvioce->addLineRight('Signature' , 22);
		$printInvioce->addDivider(' ', 20);
		$printInvioce->addLine('Thank You Please Visit', 22,true);
