...
class DataPushApiController extends Controller
{
    ...
    public function inbound_api(Request $request)
    {
        ...
        if ($request->has('store_id')) {
            $orders = Order::where(['v_id' => $v_id, 'store_id' => $request->store_id, 'date' => $current_date , 'status' => 'success'])->get();
        } else {
            $orders = Order::where(['v_id' => $v_id, 'date' => $current_date , 'status' => 'success'])->take(2)->get();
        }
        ...
        foreach($orders as $order){
            ...
            $check_sync = InvoicePush::where('order_id', $order->order_id)->where('v_id', $order->v_id)->where('store_id', $order->store_id)->latest()->first();
            ...
            if (!empty($check_sync)) {
                ...
                if ($check_sync->status == 0 || $check_sync->status == '') {
                    ...
                    $orderArr['customerAddress1']= $order->store->address1;
                    $orderArr['customerAddress2']= (isset($custAdd))?$custAdd->address2:'';
                    $orderArr['customerAddress3']= '';
                    $orderArr['customerCity']= $order->store->city;
                    $orderArr['customerDistrict']= '';
                    $orderArr['customerState']= $order->store->state;
                    $orderArr['customerPincode']= $order->store->pincode;
                ...
                $customer = User::where('c_id', $order->user_id)->first();
                    ...
                    $orderArr['IntegrationUniqueId']= $order->od_id;
                    $orderArr['customerMobileNo']= $customer->mobile;
                    $orderArr['customerFirstName']= $order->store->location.' '.$customer->first_name;
                    $orderArr['customerMiddleName']= null;
                    $orderArr['customerLastName']= $customer->last_name;
                    $orderArr['customerGender']= 'M';
                    $orderArr['customerEmail']= $customer->email;
    ...
    {
        // dd($values);
        $carts = DB::table('order_details')->select('item_id','qty','unit_mrp','discount','total','tdata')->where(['v_id' => $values->v_id, 'store_id' => $values->store_id, 'user_id' => $values->user_id, 't_order_id' => $values->od_id , 'status' => 'success'])->get();
        // dd($carts);
            $order       = [];
            $cgst_amount = 0;
            $sgst_amount = 0;
            ...
            foreach ($carts as $key => $cart) {
                ...
                foreach($p_data as $pvalue){
                    if($pvalue['item_id'] == $cart->item_id){
                        $pdata['Promo_code'] = $pvalue['promo_code'];
                        $pdata['NO']         = $pvalue['no'];
                        $pdata['PROMO_NAME'] = $pvalue['message'];
                        $pdata['START_DATE'] = date("Y-m-d H:m:i", strtotime($pvalue['start_date']));
                        $pdata['END_DATE']   = date("Y-m-d H:m:i", strtotime($pvalue['end_date']));
                        $pdata['DISCOUNT_TYPE'] = 'P';
                     }
